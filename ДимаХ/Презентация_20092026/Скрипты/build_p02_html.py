...
from fmt import fte_months, hrs, money_head, money_mln, nice_max, ratio, rub
...
def svg_timeline(timeline, k):
    ...
    parts.append('<text x="%.1f" y="%d" class="ax-title" text-anchor="middle">'
                 'Накопленная экономия часов, консервативный сценарий (+40%% к факту)</text>'
...
def svg_scenarios(scenarios):
    """Столбцы экономии часов по сценариям."""
    width, height = 700, 390
    pad_l, pad_t, pad_b = 58, 36, 78
    plot_w = width - pad_l - 20
    plot_h = height - pad_t - pad_b
    ticks = 4
    max_val = nice_max(max(sc["saved_hours"] for sc in scenarios), ticks)
    slot = plot_w / len(scenarios)
    bar_w = min(slot * 0.48, 120)

    parts = ['<svg viewBox="0 0 %d %d" class="chart" role="img" '
             'aria-label="Экономия часов по сценариям">' % (width, height)]
    for i in range(ticks + 1):
        value = max_val * i / ticks
    ...
    palette = [C_NAVY_MID, C_TEAL, C_AMBER]
    for i, sc in enumerate(scenarios):
        x = pad_l + slot * i + (slot - bar_w) / 2
        h = plot_h * sc["saved_hours"] / max_val
        y = pad_t + plot_h - h
        ...
        parts.append('<text x="%.1f" y="%.1f" class="val val-big" text-anchor="middle">%s ч</text>'
                     % (x + bar_w / 2, y - 10, hrs(round(sc["saved_hours"]))))
        parts.append('<text x="%.1f" y="%d" class="lbl" text-anchor="middle">%s</text>'
                     % (x + bar_w / 2, height - 56, esc(sc["name"])))
        parts.append('<text x="%.1f" y="%d" class="ax ax-dim" text-anchor="middle">%s</text>'
                     % (x + bar_w / 2, height - 41, esc(sc["tag"])))
        parts.append('<text x="%.1f" y="%d" class="ax ax-dim" text-anchor="middle">'
                     '%s ч/мес &#183; %s ч/год</text>'
                     % (x + bar_w / 2, height - 26,
                        hrs(round(sc["saved_hours_month"])),
                        hrs(int(round(sc["saved_hours_year"] / 10.0) * 10))))
    parts.append('<text x="%.1f" y="%d" class="ax-title" text-anchor="middle">'
                 'Часы за период</text>' % (pad_l + plot_w / 2, height - 8))
    parts.append("</svg>")
    return "".join(parts)
...
def table_rows(tasks, k):
    ...
    if rest:
        ...
        out.append(
            ...
            '<td class="t">ещё %d задач с меньшим объемом</td>'
            '<td class="c">—</td>'
            '<td class="n">%s</td>'
            '<td class="n hypo">%s</td>'
            '<td class="n save">%s</td>'
            '<td class="n">%.0f%%</td>'
            "</tr>"
            % (len(rest), hrs(round(fact)), hrs(round(without)), hrs(round(saved)),
               100.0 * saved / without)
        )
    return "\n".join(out)
    ...
    if "by_contour" not in payload and "by_component" in payload:
        ...
        for c in payload["by_component"]:
            payload["by_contour"].append({
                ...
                "tasks": c["tasks"],
                "hours": c["hours"],
                "saved_min": c.get("saved_min", 0),
                "saved_real": c.get("saved_real", 0),
            })
    return payload
...
def build(payload):
    ...
    h_mo_min = hrs(round(s["saved_hours_month_min"]))
    h_mo_max = hrs(round(s["saved_hours_month_max"]))
    h_y_min = hrs_year(s["saved_hours_year_min"])
    h_y_max = hrs_year(s["saved_hours_year_max"])
    money_y_min = money_mln(s["saved_money_year_min"])
    money_y_max = money_mln(s["saved_money_year_max"])
    fte_min = fte_share(s["fte_ongoing_min"])
    fte_max = fte_share(round(s["fte_ongoing_max"], 1))

    kpis = [
        ("%s\u2013%s" % (hrs(round(s["saved_hours_min"])), hrs(round(s["saved_hours_max"]))),
         "ч", "За %s месяцев" % period_mo_int,
         "%s \u2014 %s, %s, факт %s ч" % (
             period_from, period_to, ru_tasks(s["tasks_selected"]), hrs(s["fact_hours"]))),
        ("%s\u2013%s" % (h_mo_min, h_mo_max),
         "ч/мес", "Средний темп экономии",
         "период уже включает и тихие месяцы, и пик лета 2026"),
        ("%s\u2013%s" % (h_y_min, h_y_max),
         "ч/год", "Оценка на год",
         "средний темп \u00d7 12 \u00b7 %s\u2013%s ставки ИТ постоянно" % (fte_min, fte_max)),
        ("%s\u2013%s" % (money_y_min.replace("\u00a0млн", ""), money_y_max),
         "\u20bd/год", "Эквивалент внешней разработки",
         "ставка %s \u20bd/ч из смет Аванкор, презентация 01" % rub(s["rate"])),
    ]
    kpi_html = "\n".join(
    ...
    scen_rows = "\n".join(
        '<tr%s><td><strong>%s</strong><span class="sub">%s, +%d%% к факту</span></td>'
        '<td class="n save"><strong>%s</strong></td>'
        '<td class="n save">%s</td>'
        '<td class="n save">%s</td>'
        '<td class="n">%s</td></tr>'
        % (' class="row-hi"' if sc["id"] == "real" else "",
           esc(sc["name"]), esc(sc["tag"]), sc["add_pct"],
           hrs(round(sc["saved_hours"])),
           hrs(round(sc["saved_hours_month"])),
           hrs_year(sc["saved_hours_year"]),
           money_mln(sc["saved_money_year"]))
        for sc in payload["scenarios"]
    )

    scen_sum_rows = "\n".join(
        '<tr><td class="lab">%s<span class="sub sub--dark">%s</span></td>'
        '<td class="v %s">%s ч<span class="v-ex">%s ч/мес \u00b7 %s ч/год \u00b7 %s \u20bd/год</span></td></tr>'
        % (esc(sc["name"]), esc(sc["tag"]),
           "v-save" if sc["id"] == "real" else "v-hypo",
           hrs(round(sc["saved_hours"])),
           hrs(round(sc["saved_hours_month"])),
           hrs_year(sc["saved_hours_year"]),
           money_mln(sc["saved_money_year"]).replace("\u00a0млн", " млн"))
        for sc in payload["scenarios"]
    )

    contour_rows = "\n".join(
        "<tr><td>%s</td><td class=\"n\">%d</td><td class=\"n\">%s</td>"
        "<td class=\"n save\">%s</td><td class=\"n save\">%s</td></tr>"
        % (esc(c["contour"]), c["tasks"], hrs(c["hours"]),
           hrs(c["saved_min"]), hrs(c["saved_real"]))
        for c in payload["by_contour"]
    )

    src_rows = "\n".join(
        "<tr><td><strong>%s</strong></td><td>%s</td></tr>"
        % (esc(src.get("title") or src.get("who") or src.get("id")),
           esc(src.get("finding") or src.get("what") or ""))
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .slide--title {
    ...
    color: #f4f7fb; justify-content: center;
  }
  ...
  .kpi-num {
    font-size: clamp(1.45rem, 2.7vw, 2.35rem); font-weight: 800; letter-spacing: -.025em;
  ...
  .grid-scen { display: grid; grid-template-columns: 1.15fr 1fr; gap: 16px; align-items: start; }
  .grid-4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; }
  .card--glass {
    background: rgba(255,255,255,.07); border: 1px solid rgba(255,255,255,.16);
    box-shadow: none; margin-top: 22px; border-radius: 16px; padding: 10px 16px 6px;
  }
  ...
  .table-wrap { overflow: auto; max-height: 62vh; border: 1px solid var(--line); border-radius: 12px; background: #fff; }
  ...
  .impact {
    background: var(--paper); border: 1px solid var(--line); border-radius: 14px; padding: 14px 14px 12px;
  }
  .impact .n {
    font-size: 1.35rem; font-weight: 800; color: var(--navy); letter-spacing: -.02em; line-height: 1.1;
  }
  .impact p { margin: 6px 0 0; font-size: 12.5px; color: var(--ink-soft); line-height: 1.45; }
  .improve {
...
<div class="deck">
  <div class="slides" id="slides">
    ...
    <section class="slide slide--title">
      ...
      <p class="lead">
        За %(period_mo_int)s месяцев (%(period_from)s \u2014 %(period_to)s) внутренние задачи
        (не Аванкор) ведутся с Cursor. %(tasks_phrase)s закрыты за %(fact)s ч.
        Без ИИ тот же объем занял бы %(without_min)s&ndash;%(without_max)s ч.
        Разница &mdash; %(saved_min)s&ndash;%(saved_max)s ч за период:
        %(h_mo_min)s&ndash;%(h_mo_max)s ч каждый месяц, или %(h_y_min)s&ndash;%(h_y_max)s ч в год.
      </p>
      <div class="kpi-row">
%(kpi)s
      </div>
...
%(split)s
      ...
      <div class="src">
        ...
        %(period_from)s \u2014 %(period_to)s (%(period_mo)s мес.).
        Ставка для денежного эквивалента &mdash; %(rate)s &#8381;/ч из смет вендора (презентация 01).
      </div>
    </section>

    <section class="slide">
      <div class="eyebrow">Методика</div>
      <h2>Считаем часы, а не «магию ИИ»: факт из учета, коэффициент из исследований</h2>
      <div class="grid-2">
        <div class="card">
          ...
          <ul class="facts">
            ...
            <li><span class="b">4</span><span><strong>Деньги &mdash; не зарплата.</strong> Часы умножаем
              на ставку внешней разработки %(rate)s &#8381;, чтобы показать, сколько стоил бы этот объем
              подрядчику. Это сопоставимо с презентацией 01.</span></li>
          </ul>
        </div>
        <div>
          <div class="card">
            <p class="h4">Формула</p>
            <div class="formula">
              <div><strong>Факт</strong> = <code>часы Time Sheet</code></div>
              <div style="margin-top:10px"><strong>Без Cursor</strong> =
                <code>факт &times; k</code><br>
                <span style="color:#4a6280">k = 1,40 / 1,50 / 1,80 по сценариям</span></div>
              <div style="margin-top:10px"><strong>Экономия часов</strong> =
                <code>без Cursor &minus; факт</code></div>
              <div style="margin-top:10px"><strong>Темп</strong> =
                <code>экономия / %(period_mo)s мес.</code>
                &nbsp;&rarr;&nbsp; <code>&times; 12 = год</code></div>
            </div>
            <div class="note" style="margin-top:12px">
              <strong>Два языка процентов.</strong> «+40%% к факту» (k = 1,40) &mdash; это 29%% ускорения,
              а не «на 40%% быстрее». «На 55%% быстрее» из лабораторного GitHub Copilot дало бы k = 2,26.
              Такой коэффициент мы не берем: это верхняя лабораторная оценка, не портфель задач 1С.
              Год &mdash; экстраполяция среднего темпа за %(period_mo)s мес., а не отдельный факт.
            </div>
          </div>
        </div>
      </div>
      <div class="card" style="margin-top:14px">
        <p class="h4">Откуда коэффициенты</p>
        <table>
          <thead><tr><th>Источник</th><th>Что показали</th></tr></thead>
          <tbody>
%(src_rows)s
          </tbody>
        </table>
    ...
    <section class="slide">
      ...
      <div class="legend">
        <span><i style="background:%(navy_mid)s"></i>Прогноз без Cursor (консервативно, +40%%)</span>
...
%(bars)s
      ...
      <div class="src">
        Показаны %(top_bars)d крупнейших задач (около двух третей объема). Диаграмма &mdash;
        консервативный сценарий (+40%% к факту). Реалистичный увеличивает прогноз без ИИ до +80%%.
      </div>
    </section>
...
%(rows)s
            <tr class="total">
              <td colspan="3">ИТОГО по %(tasks)d задачам, консервативный сценарий</td>
              <td class="n">%(fact)s</td>
              <td class="n">%(without_min)s</td>
              <td class="n">%(saved_min)s</td>
              <td class="n">%(pct_min)s%%</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div class="src">
        «Без Cursor» = факт &times; 1,40. Доля экономии в гипотетическом объеме постоянна по задачам
        (%(pct_min)s%%) &mdash; коэффициент общий. Различается абсолют: чем больше задача, тем больше часов возвращается.
      </div>
    </section>
...
%(contours)s
        ...
        <div class="card">
          <p class="h4">Разрез по контурам, консервативный / реалистичный</p>
          <table>
            <thead>
              <tr>
                <th>Контур</th><th class="n">Задач</th><th class="n">Факт, ч</th>
                <th class="n">Экон. min, ч</th><th class="n">Экон. max, ч</th>
              </tr>
            </thead>
            <tbody>
%(contour_rows)s
          ...
          <div class="note" style="margin-top:12px">
            ...
            %(h_mo_min)s&ndash;%(h_mo_max)s ч/мес уже учитывает и зиму, и пик лета 2026:
    ...
    <section class="slide">
      ...
      <h2>Три сценария: от нижней границы ТЗ до McKinsey по новому коду</h2>
      <p class="lead">
        Единственная цифра всегда спорна. Поэтому &mdash; диапазон, и каждая оценка
        приведена к месяцу и году. GitHub Copilot 55,8%% быстрее сознательно не используем:
        это лаборатория, не наш портфель.
      </p>
      <div class="grid-scen">
        <div class="card">
%(scenarios_chart)s
        ...
        <div class="card">
          ...
          <table>
            <thead>
              <tr>
                <th>Сценарий</th>
                <th class="n">За %(period_mo_int)s мес., ч</th>
                <th class="n">Ч/мес</th>
                <th class="n">Ч/год</th>
                <th class="n">&#8381;/год</th>
              </tr>
...
%(scen_rows)s
          ...
          <div class="note" style="margin-top:12px">
            <strong>Почему реалистичный все еще осторожен.</strong> McKinsey дает 35&ndash;45%% экономии
            времени на написание нового кода. +80%% к факту (k = 1,80) &mdash; это 44%% ускорения,
            верх диапазона McKinsey. Лабораторный GitHub 55,8%% (k = 2,26) отбрасываем.
          </div>
        </div>
      </div>
      <div class="src">
        Консервативный сценарий совпадает с формулировкой ТЗ «+40%% к фактическому времени».
        Базовый &mdash; «+50%%». Реалистичный &mdash; «+80%% к факту», то есть «на 44%% быстрее»
        по верхней границе McKinsey для нового кода. Часы в месяц = экономия за период /
        %(period_mo)s мес.; год = месяц &times; 12.
      </div>
    </section>
    ...
    <section class="slide">
      ...
      <div class="grid-4">
        <div class="impact">
          ...
          <div class="n">&times;%(cap_min)s&ndash;%(cap_max)s</div>
          <p>Та же команда закрывает в 1,4&ndash;1,8 раза больший объем работ за тот же календарный срок.</p>
        </div>
        <div class="impact">
          <p class="h4">Темп без найма</p>
          <div class="n">%(h_mo_min)s&ndash;%(h_mo_max)s ч/мес</div>
          <p>%(fte_min)s&ndash;%(fte_max)s ставки ИТ постоянно. Штат не растет,
            подрядчик не вызывается, тот же состав закрывает больший объем.</p>
        </div>
        <div class="impact">
          <p class="h4">Оценка на год</p>
          <div class="n">%(h_y_min)s&ndash;%(h_y_max)s ч</div>
          <p>если средний темп сохранится: %(money_y_min)s&ndash;%(money_y_max)s &#8381;/год
            по ставке вендора. Пик лета уже внутри среднего.</p>
        </div>
        <div class="impact">
          <p class="h4">Приоритеты бизнеса</p>
          <div class="n">%(share_opt)s%%</div>
      ...
      <div class="card" style="margin-top:16px">
        ...
        <p style="margin:0;font-size:14px;color:var(--ink-soft);line-height:1.55">
          ...
          которых не пришлось тратить. Складывать 1,2&ndash;2,2 млн за вендора и
          %(money_y_min)s&ndash;%(money_y_max)s в год внутренней емкости напрямую нельзя:
          это разные контуры. Вместе они показывают, что Cursor работает и наружу, и внутрь.
        </p>
      </div>
      <div class="src">
        ...
        <div>
          <ul class="facts">
            <li><span class="b">1</span><span><strong>За %(period_mo_int)s месяцев высвобождено
              %(saved_min)s&ndash;%(saved_max)s ч ИТ</strong> на внутренних задачах
              (%(period_from)s \u2014 %(period_to)s). Cursor сокращает цикл, а не качество.</span></li>
            <li><span class="b">2</span><span><strong>Это %(h_mo_min)s&ndash;%(h_mo_max)s ч каждый месяц
              и %(h_y_min)s&ndash;%(h_y_max)s ч в год</strong> &mdash; %(fte_min)s&ndash;%(fte_max)s ставки
              без найма. Штат не растет, подрядчик не вызывается, бэклог идет быстрее.</span></li>
            <li><span class="b">3</span><span><strong>Нижняя граница честная:</strong> +40%% к факту
              (29%% ускорения) ниже лабораторного GitHub и внутри диапазона McKinsey по рефакторингу.</span></li>
            <li><span class="b">4</span><span><strong>Верх не фантазия:</strong> +80%% к факту = «на 44%%
              быстрее» по верхней границе McKinsey для нового кода. 55,8%% GitHub в расчет не входят.</span></li>
          </ul>
        </div>
        <div>
          <div class="card" style="background:rgba(255,255,255,.07);border-color:rgba(255,255,255,.16)">
            <p class="h4" style="color:rgba(226,234,244,.6)">Экономия часов: период, месяц, год</p>
            <table class="sum">
              <tbody>
%(scen_sum_rows)s
...
%(split)s
      ...
      <div class="src">
        ...
        горизонт %(period_from)s \u2014 %(period_to)s (%(period_mo)s мес.).
        Коэффициенты &mdash; GitHub Copilot 2023, McKinsey 2023, Google enterprise RCT 2024, IBM/McKinsey 2024.
...
""" % {
        ...
        "kpi": kpi_html,
        "split": svg_split(s["fact_hours"], s["saved_hours_min"],
        ...
        "without_min": hrs(round(sc_min["without_hours"])),
        "without_max": hrs(round(sc_real["without_hours"])),
        "saved_min": hrs(round(s["saved_hours_min"])),
        "saved_max": hrs(round(s["saved_hours_max"])),
        "saved_min_r": hrs(round(s["saved_hours_min"])),
        "pct_min": "%.0f" % sc_min["time_reduction_pct"],
        "rate": rub(s["rate"]),
        "top_bars": TOP_BARS,
        "cap_min": ratio(sc_min["capacity_x"]),
        "cap_max": ratio(sc_real["capacity_x"]),
        "fte_min": fte_min,
        "fte_max": fte_max,
        "cost_min": money_head(s["saved_cost_min"]),
        "cost_max": money_head(s["saved_cost_max"]),
        ...
        "h_y_min": h_y_min,
        "h_y_max": h_y_max,
        "money_y_min": money_y_min,
        "money_y_max": money_y_max,
        "share_opt": "%.0f" % (100.0 * next(
            t["hours"] for t in payload["by_type"] if t["issue_type"] == "Change Request"
        ) / s["fact_hours"]),
    }
    return html
