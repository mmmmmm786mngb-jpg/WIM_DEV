...
# -*- coding: utf-8 -*-
"""
Sverka NDFL: etalon 27292 vs PoNovomu4 (Locks + NkdCoupon + NkdParam).
...
OUT_HTML = os.path.join(DOC, "imdev7330_ndfl_old_vs_new4_diff.html")
OUT_JSON = os.path.join(DOC, "imdev7330_ndfl_old_vs_new4_diff.json")

FILES = {
    "uk_old": os.path.join(BASE, "НДФЛ_Управление_27292.xlsx"),
    "uk_n4": os.path.join(BASE, "НДФЛ_Управление_27292_ПоНовому4.xlsx"),
    "pf_old": os.path.join(BASE, "НДФЛ_Портфели_27292.xlsx"),
    "pf_n4": os.path.join(BASE, "НДФЛ_Портфели_27292_ПоНовому4.xlsx"),
    "msg_old": os.path.join(BASE, "Сообщения_поСтарому_Аванкор.txt"),
    "msg_n4": os.path.join(BASE, "Сообщения_поНовому4_Аванкор.txt"),
    "log_n4": os.path.join(BASE, "Лог формирования начисдений НДФЛ_ПоНовому4.xlsx"),
}
...
def analyze():
    ...
    safe_print("Parse log new4 xlsx...")
...
def write_html(r):
    ...
    others_ok = (
        cmpw.get("diff_sums", 1) == 0
        and cmpw.get("only_old_n", 1) == 0
        and cmpw.get("only_new_n", 1) == 0
    )
    struct_wo = cmpw.get("diff_structure_only", 0)

    if pf_ok and plat_ok and others_ok and struct_wo == 0:
        verdict = (
        ...
        vcls = "warn"
    elif pf_ok and plat_ok and not others_ok:
...
<head>
<meta charset="utf-8">
<title>IMDEV-7330. Сверка эталон vs ПоНовому4</title>
<style>
...
<header>
<div class="wrap">
    <h1>IMDEV-7330. Сверка эталон 27292 vs ПоНовому4</h1>
    <div class="sub">
...
<tr><td>УК новое4</td><td>{escape(fname("uk_n4"))}</td><td class="num">{fbytes("uk_n4")}</td><td class="sig">{escape(r["md5"].get("uk_n4",""))}</td></tr>
...
<tr><td>Портфели новое4</td><td>{escape(fname("pf_n4"))}</td><td class="num">{fbytes("pf_n4")}</td><td class="sig">{escape(r["md5"].get("pf_n4",""))}</td></tr>
</tbody>
</table>
...
<table>
<thead><tr><th>Показатель</th><th>Эталон</th><th>Новое4</th><th>Дельта</th></tr></thead>
<tbody>
...
<table><thead><tr><th>Поле</th><th>Эталон</th><th>Новое4</th><th>Дельта</th></tr></thead><tbody>{pf_totals_html}</tbody></table>
'''}

<h2>3. УК: итог по всем клиентам</h2>
<table>
<thead><tr><th>Показатель</th><th>Эталон</th><th>Новое4</th></tr></thead>
<tbody>
...
<table>
<thead><tr><th>Поле</th><th>Эталон</th><th>Новое4</th><th>Дельта</th></tr></thead>
...
<h2>6. Сообщения</h2>
<table>
<thead><tr><th>Тип</th><th>Эталон</th><th>Новое4</th><th>Дельта</th></tr></thead>
<tbody>{"".join(kind_rows)}</tbody>
</table>
<p>Клиентов «невозможно распределить»: эталон {msg["clients_rasp_a"]}, новое4 {msg["clients_rasp_b"]}.</p>
<p>Уникальных строк только в эталоне: {msg["only_a_n"]}, только в новом: {msg["only_b_n"]}.</p>

<h2>7. Блокировки в логе</h2>
{lock_lis}

<footer>Скрипт compare_ndfl_old_vs_new4.py. JSON рядом с этим HTML.</footer>
</div>
</body>
</html>
"""
...
def print_summary(r):
    ...
    cmpu = uk["compare"]
    cmpw = uk["compare_wo_plat"]
    safe_print("==== SUMMARY old vs new4 ====")
