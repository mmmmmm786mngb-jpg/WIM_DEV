...
- [Ext/ManagerModule.bsl](../Расширения/NDFL_RDU/DataProcessors/ФормированиеНачисленийНДФЛ/Ext/ManagerModule.bsl): область `IMDEV7330_ПараллельноеФормированиеНДФЛ_Менеджер`; оркестратор, снимок, `NDFL_RDU_РазбитьСтрокиНаПачки(ИндексыСтрок, КоличествоПачекПользователя, СнимокПолнойТЧ = Неопределено)` и пр. — см. [imdev7330_ndfl_longops_n_batches_spec.html](imdev7330_ndfl_longops_n_batches_spec.html) (п. 3.2 — геометрия окон).
- [Ext/ObjectModule.bsl](../Расширения/NDFL_RDU/DataProcessors/ФормированиеНачисленийНДФЛ/Ext/ObjectModule.bsl): перехваты и кэш (`#Область ПРОГРАММНЫЙ_ИНТЕРФЕЙС`, `#Область СлужебныеПроцедурыИФункции`); воркер пачки — `#Область IMDEV7330_ПараллельноеФормированиеНДФЛ_Объект`.
...
- HTML-ТЗ [imdev7330_ndfl_longops_n_batches_spec.html](imdev7330_ndfl_longops_n_batches_spec.html) синхронизировано с менеджером (в т.ч. п. 3.2 — разбиение), диаграмма п. 7.
...
**Параметры пачки (структура для воркера, сериализация в XML):** поля `НомерПачки`, `Шапка` (структура реквизитов), `СтрокиДатыДокументов` (`ТаблицаЗначений`), `АдресРезультатаПачки`. Число пачек `ВсегоПачек` для наименования ФЗ передаётся отдельным аргументом в `NDFL_RDU_ЗапуститьПачкуФормированияНДФЛЧерезБСП`. `ИдентификаторФормы` передаётся в `NDFL_RDU_ПодготовитьПараметрыПачки` только для создания адреса ВХ и в XML не входит.
...
5. **`NDFL_RDU_НачатьОжидание` (`&НаКлиенте`):** `ПараметрыОжидания`, `ВыводитьОкноОжидания`, `ВыводитьПрогрессВыполнения`, `ОписаниеОповещения` на `NDFL_RDU_ОбработатьРезультат` (ТЗ п. 8.3 БСП).
6. **`NDFL_RDU_ОбработатьРезультат`:** по статусу вызов `NDFL_RDU_ОбработатьИтогНаСервере(РезультатДО, Истина или Ложь)`; вывод `ТабличныйДокумент` через `NDFL_RDU_ПоказатьТабличныйДокументЛога`; обработать `Статус` «Выполнено» сразу (файловая ИБ), «Ошибка» — `NDFL_RDU_ПоказатьОшибку` (ТЗ п. 8.1, 10).
...
### 6.2. `NDFL_RDU_РазбитьСтрокиНаПачки` (факт реализации IMDEV-7330)

**Актуально:** сигнатура с третьим параметром `СнимокПолнойТЧ`; геометрия окон по `1..Макс(НомерСтроки)`, раскладка **одним проходом** по `ИндексыСтрок`, слияние соседних пачек с общим клиентом (`NDFL_RDU_СлитьСоседниеПачкиПоОбщемуКлиенту`), верхняя граница `K` (50), порог «одна пачка» при **менее двух** отобранных строк — см. [imdev7330_ndfl_longops_n_batches_spec.html](imdev7330_ndfl_longops_n_batches_spec.html), п. 3.2.

Ниже — черновик этапа планирования (часть формулировок не совпала с итоговым кодом: порог был `2 * K`, упаковка «по клиенту» иная — через окна + слияние на границе):

- Геометрия до **K** окон по диапазону номеров строк полной ТЧ и пересечение с отобранными индексами (в коде — эквивалент через один проход по `ИндексыСтрок`);
- Пустые окна в результат не включаются;
- После окон — слияние соседних пачек при общем клиенте на границе;
- Возврат: массив массивов номеров строк; `NDFL_RDU_ПодготовитьПараметрыПачки(Снимок, НомерПачки, СтрокиПачки, ИдентификаторФормы)` принимает один такой внутренний массив на пачку.

### 6.3. `NDFL_RDU_ПодготовитьПараметрыПачки`

Собрать структуру для XML воркера: `НомерПачки`, `Шапка`, `СтрокиДатыДокументов` (копия строк по индексам из исходной ТЧ), `АдресРезультатаПачки` (ВХ обёртки; `ИдентификаторФормы` — только аргумент функции при `ПоместитьВоВременноеХранилище`, не поле структуры).
