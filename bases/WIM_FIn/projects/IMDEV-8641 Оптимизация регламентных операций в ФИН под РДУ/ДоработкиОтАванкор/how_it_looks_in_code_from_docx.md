...
Для Каждого СтрокаДня Из ТаблицаДней Цикл
    ...
    Если ОбработкаДопДействий <> Неопределено Тогда
        Попытка
            ...
            ПараметрыОбработки.Вставить("ПланРегламентныхОпераций", тПланРеглОперацийПоДням);
...
| `Дата` | день плана |
| `ПланРегламентныхОпераций` | таблица плана на день (в `Релиз___2_8_5_0.txt` опечатка: `ПланРегламентныхОпераци`) |
...
Процедура ПриФормированииПланаРегламентныхОперацийЗаДень(ПараметрыОбработки, Отказ = Ложь)
    ...
    ПараметрыОбработки.ПланРегламентныхОпераций.Очистить();
...
По инструкции `Релиз___2_8_5_0.txt` для RDU-обработки нужно **убрать** пустые ветки «Перед/После всех регламентных операций» в `ВыполнитьДополнительныеДействия` и заполнить `ПриФормированииПланаРегламентныхОперацийЗаДень` логикой из менеджера РС.
