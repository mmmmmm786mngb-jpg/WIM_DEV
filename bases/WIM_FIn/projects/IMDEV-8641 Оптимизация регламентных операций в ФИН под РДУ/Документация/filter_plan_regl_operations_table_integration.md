...
1. **Константа `ПулРозницаДУ` заполнена** — непустая **СправочникСсылка.Пул** (эталон пула «розница ДУ»). В метаданных заводится константа с именем **`ПулРозницаДУ`** (в текущей выгрузке WIM_FIn может отсутствовать до доработки).
