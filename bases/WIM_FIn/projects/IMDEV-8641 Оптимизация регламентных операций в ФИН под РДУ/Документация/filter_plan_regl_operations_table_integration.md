...
**Принцип отбора строк:** логика шагов 1–3 — в **«Алгоритм отсеивания строк»**; технический порядок запросов и вызовов — в **«Реализация»**; **перечень имён процедур** для белого списка задаётся **в коде** (см. п. **3.1**); **тексты запросов окончательной функции** п. **3.2** — в отдельном описании / ТЗ на доработку.
...
flowchart TD
    ...
    I --> J[Истина и таблица портфелей для сохранения расчёта, может быть пустая]
    J --> K[По строкам плана: Портфель не в таблице ответа, тогда Выполнять = 0]
    K --> E
```

Узел **«Нет»** у проверки белого списка соответствует возврату **`Ложь`** из внешней функции: по этому виду строки плана **не меняются**, цикл переходит к следующему виду. Узел **«Да»** — имя исполняемой процедуры вида входит в список, вызывается п. **3.2** и правка **`Выполнять`**. Если таблица портфелей из ответа **пустая**, на шаге применения обнуляются **все** строки плана с данным видом (см. п. **3.1**).
...
#### 3.1. Внешняя функция (вид операции, день, белый список имён процедур)

**Назначение:** решить, участвует ли данный **`ВидОперации`** в оптимизации по РДУ (по **имени исполняемой процедуры**), и при положительном ответе получить перечень портфелей, для которых расчёт **оставляем** (остальным с этим видом — **`Выполнять = 0`**).
...
1. **Белый список** задаётся **в коде** как фиксированный перечень **строк** в формате **`ИсполняемыеПроцедурыЗакрытияПериода.ИмяПроцедуры`** — то есть **с префиксом имени общего модуля**, как при обращении к процедуре в коде. Элементы списка **должны в точности совпадать** по строке с реквизитом справочника (см. п. 2).
2. **Связь со справочником:** в информационной базе реквизит **`Справочник.ВидыОперацийЗакрытияПериода.ИсполняемаяПроцедура`** (тип **Строка**, в метаданных WIM_FIn — до 200 символов) хранит **полное имя с префиксом модуля** — **`ИсполняемыеПроцедурыЗакрытияПериода.ИмяПроцедуры`**, в том же виде, как в данных и в **`ПолучитьСписокОперацийДляВыполнения`**. Проверка «имя в белом списке»: **`ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидОперации, "ИсполняемаяПроцедура")`** (или эквивалент) и сравнение с элементами зашитого в коде перечня (**полное совпадение** строки, **регистр** — как в ИБ; пустая **`ИсполняемаяПроцедура`** — в ТЗ: считать «не в списке» или отдельная ветка).
3. Если строка **`ИсполняемаяПроцедура`** для данного **`ВидОперации`** **не входит** в белый список — возврат **`Структура`**: флаг **`Ложь`**. **Второй ключ не используется.** По этому виду операции строки плана **не меняем**.
4. Если **входит** — вызывается **окончательная функция** (п. **3.2**) с параметрами **`МассивПортфелей`**, **`День`**, **`ВидОперации`**. По её результату формируется возврат **`Структура`**: флаг **`Истина`** и **второй ключ** — **таблица портфелей** (например **`ТаблицаЗначений`** с колонкой **`Портфель`**) **или** пустая таблица / пустой набор строк.

**Смысл второго ключа при `Истина`:** портфели, перечисленные в таблице результата окончательной функции, — те, по которым **`Выполнять`** в плане **не трогаем**; по **всем остальным** портфелям с тем же **`ВидОперации`** (в пределах исходного плана) — **`Выполнять = 0`**.  
Если окончательная функция вернула **пустой** список портфелей — **ни один** портфель не попал в перечень «оставить расчёт», значит для **всех** строк плана с данным **`ВидОперации`** (и **`Выполнять <> 0`** до правки) выставляем **`Выполнять = 0`**.

#### 3.2. Окончательная функция (портфели по операции, день, вид операции)

**Назначение:** по конкретной комбинации **`ВидОперации`** (уже отобранной по **`ИсполняемаяПроцедура`** и белому списку имён), **`Дня`** и ограниченному **`МассивПортфелей`** из плана выполнить **специализированные запросы** и вернуть подмножество портфелей, для которых операцию **оставляем в исполнении**.
...
- На выходе — **либо пустой** результат (нет строк / пустая таблица портфелей), **либо** перечень **`Портфель`**, которые должны **остаться** с исполнением (**`Выполнять`** не обнуляем). Формат выхода согласовать с внешней функцией (таблица с колонкой **`Портфель`** или **`Массив`** — единообразно в коде).

**Уточнение:** для **разных** имён из белого списка (разных **`ИсполняемаяПроцедура`** / **`ВидОперации`**) внутри окончательной функции допускается **ветвление** (`Если ВидОперации = ... Тогда` / `Если ИмяПроцедуры = ...` / отдельные процедуры на вид) — если у операций различаются источники данных; общий контракт входа/выхода сохраняется.

#### 3.3. Применение результата к таблице плана

Цикл по видам из шага **2** выполняется **в коде**; для каждого вида — вызов **внешней** функции п. **3.1** с **`ВидОперации`**, **`День`** и собранным **`МассивПортфелей`**, затем обновление **исходной** **`ТаблицаЗначений`** плана по правилам п. **3.1** (флаг **`Ложь`** — пропуск вида; **`Истина`** + таблица портфелей — обнуление **`Выполнять`** у «лишних» портфелей).
...
- **`CommonModules/ИсполняемыеПроцедурыЗакрытияПериода`** — эталон **имён процедур** для белого списка в коде.
- **`Catalogs/ВидыОперацийЗакрытияПериода`**, реквизит **`ИсполняемаяПроцедура`** — в ИБ хранится **с префиксом модуля**: **`ИсполняемыеПроцедурыЗакрытияПериода.ИмяПроцедуры`**; белый список в коде задаёт те же строки **полным совпадением**.
