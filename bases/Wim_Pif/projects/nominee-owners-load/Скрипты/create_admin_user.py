...
# -*- coding: utf-8 -*-
"""
Create IB user admin / 1 in WIM_PIF: full roles, no dangerous-actions warning,
catalog Пользователи + group Администраторы.
"""
...
USER_NAME = "admin"
USER_PASSWORD = "1"
CONN_ANON = "Srvr='localhost';Ref='WIM_PIF';App='PyCOM';Locale=ru_RU;"
...
ROLES = [
    ...
    "ВТБ_ОткрытиеВнешнихОтчетовОбработок",
    "ИспользованиеДополнительныхОтчетовИОбработок",
...
def find_ib_user(conn, name):
    """Find IB user by name; None if missing."""
    found = conn.ПользователиИнформационнойБазы.НайтиПоИмени(name)
    if user_name_or_empty(found) == name:
        return found
    return None


def assign_roles(conn, ib_user):
    """Replace IB roles with the admin set."""
    try:
    ...
    for role_name in ROLES:
        role = conn.Метаданные.Роли.Найти(role_name)
        if role is None:
...
def main():
    ...
    try:
        ...
        safe_print("USERS_BEFORE=%s" % ",".join(before) if before else "USERS_BEFORE=<none>")
        ...
        added, missing = assign_roles(conn, ib_user)
        safe_print("ROLES=%s" % ",".join(added))
        if missing:
            safe_print("ROLES_MISSING=%s" % ",".join(missing))
        ...
        roles_now = []
        for role in cur.Роли:
            try:
                roles_now.append(role.Имя)
            except Exception:
