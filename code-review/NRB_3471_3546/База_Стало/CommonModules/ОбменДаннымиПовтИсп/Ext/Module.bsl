...
Функция ПолучитьСписокПлановОбменаКонфигурации() Экспорт
	...
	Для Каждого ПланОбмена ИЗ Метаданные.ПланыОбмена Цикл
		...
		// © ВТБКБ  - Начало
		// Обмен данными не с 1С обрабатывается отдельно
		Если ПланОбмена.Имя = "ОбменКодамиКлиентов" Тогда
			Продолжить;
		КонецЕсли;
		// © ВТБКБ - Конец
...
Функция ИспользуемыеТранспортыСообщенийОбмена(УзелИнформационнойБазы) Экспорт
	...
	Возврат ПланыОбмена[ПолучитьИмяПланаОбмена(УзелИнформационнойБазы)].ИспользуемыеТранспортыСообщенийОбмена();
...
Функция ПравилаРегистрацииОбъекта(Знач ИмяПланаОбмена, Знач ПолноеИмяОбъекта) Экспорт
	...
	ПравилаРегистрацииОбъектовПланаОбменаПоОдномуОбъектуМетаданных = ПравилаРегистрацииОбъектовПланаОбмена.Скопировать(Новый Структура("ОбъектМетаданныхИмя", ПолноеИмяОбъекта));
	
	Возврат ПоместитьВоВременноеХранилище(ПравилаРегистрацииОбъектовПланаОбменаПоОдномуОбъектуМетаданных, Новый УникальныйИдентификатор);
