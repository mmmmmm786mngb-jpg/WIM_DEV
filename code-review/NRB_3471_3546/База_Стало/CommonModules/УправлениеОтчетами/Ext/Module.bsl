...
// 
// Параметры
// ВыбВидСравнения       - выбранный вид сравнения (системное перечисление), тип - системные перечисления "ВидСравнения" или "ВидСравненияКомпоновкиДанных"
// ЗначениеОтбора        - Значение отбора запроса строкой с префиксом "&"
// СтрокаПоляОтбора 	 - Строка, Аргумент запроса, т.е. то что должно быть написано до вида сревнения
// ЗначениеОтбораС       - Произвольный, значение отбора для видов сравнения, у которых необходимо указывать два значения, начальное значение
// ЗначениеОтбораПо      - Произвольный, значение отбора для видов сравнения, у которых необходимо указывать два значения, конечное значение
// 
// Возвращаемое значение 
//  Строка для запроса
//
Функция ПолучитьСтрокуОтбора(ВыбВидСравнения, ЗначениеОтбора, СтрокаПоляОтбора, ЗначениеОтбораС = Неопределено,
	ЗначениеОтбораПо = Неопределено, СамоЗначение = Неопределено, СамоЗначениеС = Неопределено,
	СамоЗначениеПо = Неопределено) Экспорт

	Если ТипЗнч(ВыбВидСравнения) = Тип("ВидСравненияКомпоновкиДанных") Тогда
		ВС  = ВидСравненияКомпоновкиДанных;
		СКД = Истина;
	Иначе	
		ВС  = ВидСравнения;
		СКД = Ложь;
	...
	ЗначениеОтбора = ?(Лев(ЗначениеОтбора,1)="&", ЗначениеОтбора, "&"+ЗначениеОтбора);
	СтрокаВозврата = "";
	
	Если ВыбВидСравнения = ВС.Равно Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " = " + ЗначениеОтбора;
		
	ИначеЕсли ВыбВидСравнения = ВС.НеРавно Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " <> " + ЗначениеОтбора;
		
	ИначеЕсли ВыбВидСравнения = ВС.ВСписке Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " В (" + ЗначениеОтбора + ")";
		
	ИначеЕсли ВыбВидСравнения = ВС.ВСпискеПоИерархии ИЛИ ВС = ВидСравнения.ВИерархии Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " В ИЕРАРХИИ (" + ЗначениеОтбора + ")";
		
	ИначеЕсли ВыбВидСравнения = ВС.НеВСписке Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " НЕ В (" + ЗначениеОтбора + ")";
		
	ИначеЕсли ВыбВидСравнения = ВС.НеВСпискеПоИерархии ИЛИ ВС = ВидСравнения.НеВИерархии Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " НЕ В ИЕРАРХИИ (" + ЗначениеОтбора + ")";
		
	ИначеЕсли ВыбВидСравнения = ВС.Больше Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " > " + ЗначениеОтбора;
		
	ИначеЕсли ВыбВидСравнения = ВС.БольшеИлиРавно Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " >= " + ЗначениеОтбора;
		
	ИначеЕсли ВыбВидСравнения = ВС.Меньше Тогда
		Если НЕ (ТипЗнч(СамоЗначение) = Тип("Дата") И ОбщегоНазначения.ЗначениеНеЗаполнено(СамоЗначение)) Тогда
			СтрокаВозврата = СтрокаПоляОтбора + " < " + ЗначениеОтбора;
		КонецЕсли; 
		
	ИначеЕсли ВыбВидСравнения = ВС.МеньшеИлиРавно Тогда
		Если НЕ (ТипЗнч(СамоЗначение) = Тип("Дата") И ОбщегоНазначения.ЗначениеНеЗаполнено(СамоЗначение)) Тогда
			СтрокаВозврата = СтрокаПоляОтбора + " <= " + ЗначениеОтбора;
		КонецЕсли; 
		
	ИначеЕсли ВыбВидСравнения = ВС.Содержит Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " ПОДОБНО " + "%"+ЗначениеОтбора+"%";
		
	ИначеЕсли ВыбВидСравнения = ВС.НеСодержит Тогда
		СтрокаВозврата = СтрокаПоляОтбора + " НЕ ПОДОБНО " + "%"+ЗначениеОтбора+"%";	
		
	// УНИКАЛЬНЫЕ ВИДЫ СРАВНЕНИЙ	
	
	// значение "Заполнено" существует только у видов сравнений компоновки данных
	ИначеЕсли СКД И ВыбВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено Тогда
		СтрокаВозврата = "(" + СтрокаПоляОтбора + " НЕ ЕСТЬ NULL ИЛИ " + СтрокаПоляОтбора + " <> НЕОПРЕДЕЛЕНО ИЛИ " + СтрокаПоляОтбора + " <> " + ПолучитьПустоеПредставлениеЗначенияДляЗапроса(СамоЗначение) + ")";
		
	ИначеЕсли СКД И ВыбВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено Тогда
		СтрокаВозврата = "(" + СтрокаПоляОтбора + " ЕСТЬ NULL ИЛИ " + СтрокаПоляОтбора + " = НЕОПРЕДЕЛЕНО ИЛИ " + СтрокаПоляОтбора + " = "+ ПолучитьПустоеПредставлениеЗначенияДляЗапроса(СамоЗначение) + ")";
	
	// значения "Интервал" существуют только у видов сравнений
	ИначеЕсли НЕ СКД И ВыбВидСравнения = ВидСравнения.Интервал Тогда
		Если ТипЗнч(СамоЗначениеПо) = Тип("Дата") И ОбщегоНазначения.ЗначениеНеЗаполнено(СамоЗначениеПо) Тогда
			СтрокаВозврата = "(" + СтрокаПоляОтбора + " > " + ЗначениеОтбораС + ")";
		Иначе
			СтрокаВозврата = "(" + СтрокаПоляОтбора + " > " + ЗначениеОтбораС + " И " + СтрокаПоляОтбора + " < " + ЗначениеОтбораПо + ")";
		КонецЕсли;
		
	ИначеЕсли НЕ СКД И ВыбВидСравнения = ВидСравнения.ИнтервалВключаяГраницы Тогда
		Если ТипЗнч(СамоЗначениеПо) = Тип("Дата") И ОбщегоНазначения.ЗначениеНеЗаполнено(СамоЗначениеПо) Тогда
			СтрокаВозврата = "(" + СтрокаПоляОтбора + " >= " + ЗначениеОтбораС + ")";
		Иначе
			СтрокаВозврата = "(" + СтрокаПоляОтбора + " >= " + ЗначениеОтбораС + " И " + СтрокаПоляОтбора + " <= " + ЗначениеОтбораПо + ")";
	...
	ИначеЕсли НЕ СКД И ВыбВидСравнения = ВидСравнения.ИнтервалВключаяНачало Тогда
		Если ТипЗнч(СамоЗначениеПо) = Тип("Дата") И ОбщегоНазначения.ЗначениеНеЗаполнено(СамоЗначениеПо) Тогда
			СтрокаВозврата = "(" + СтрокаПоляОтбора + " >= " + ЗначениеОтбораС + ")";
		Иначе
			СтрокаВозврата = "(" + СтрокаПоляОтбора + " >= " + ЗначениеОтбораС + " И " + СтрокаПоляОтбора + " < " + ЗначениеОтбораПо + ")";
	...
	ИначеЕсли НЕ СКД И ВыбВидСравнения = ВидСравнения.ИнтервалВключаяОкончание Тогда
		Если ТипЗнч(СамоЗначениеПо) = Тип("Дата") И ОбщегоНазначения.ЗначениеНеЗаполнено(СамоЗначениеПо) Тогда
			СтрокаВозврата = "(" + СтрокаПоляОтбора + " > " + ЗначениеОтбораС + ")";
		Иначе
			СтрокаВозврата = "(" + СтрокаПоляОтбора + " > " + ЗначениеОтбораС + " И " + СтрокаПоляОтбора + " <= " + ЗначениеОтбораПо + ")";
...
// функция возвращает пустое представление (пустую ссылку) значения для подстановки в текст запроса
Функция ПолучитьПустоеПредставлениеЗначенияДляЗапроса(ВыбЗначение)
	
	Если ТипЗнч(ВыбЗначение) = Тип("Дата") Тогда
		Возврат "ДАТАВРЕМЯ(1,1,1)";
		
	ИначеЕсли ТипЗнч(ВыбЗначение) = Тип("Число") Тогда	
		Возврат "0";
		
	ИначеЕсли ТипЗнч(ВыбЗначение) = Тип("Строка") Тогда	
		Возврат "";
		
	ИначеЕсли Найти(СокрЛП(ТипЗнч(ВыбЗначение)), "Справочник") > 0 Тогда	
		Возврат "ЗНАЧЕНИЕ(Справочник." + СокрЛП(ВыбЗначение.Метаданные().Имя) + ".ПустаяСсылка)"; 
		                       
	ИначеЕсли Найти(СокрЛП(ТипЗнч(ВыбЗначение)), "ПланВидовХарактеристик") > 0 Тогда	
		Возврат "ЗНАЧЕНИЕ(ПланВидовХарактеристик." + СокрЛП(ВыбЗначение.Метаданные().Имя) + ".ПустаяСсылка)"; 	
	
	ИначеЕсли Найти(СокрЛП(ТипЗнч(ВыбЗначение)), "Перечисление") > 0 Тогда	
		Возврат "ЗНАЧЕНИЕ(Перечисление." + СокрЛП(ВыбЗначение.Метаданные().Имя) + ".ПустаяСсылка)"; 
		
	ИначеЕсли Найти(СокрЛП(ТипЗнч(ВыбЗначение)), "Документ") > 0 Тогда	
		Возврат "ЗНАЧЕНИЕ(Документ." + СокрЛП(ВыбЗначение.Метаданные().Имя) + ".ПустаяСсылка)"; 	
		
	ИначеЕсли Найти(СокрЛП(ТипЗнч(ВыбЗначение)), "БизнесПроцесс") > 0 Тогда	
		Возврат "ЗНАЧЕНИЕ(БизнесПроцесс." + СокрЛП(ВыбЗначение.Метаданные().Имя) + ".ПустаяСсылка)"; 		
...
//
Процедура ЗаполнитьОтборИзТаблицыЗначений(Отбор, Таблица) Экспорт
...
Функция ПроверитьПовторыИзмеренийПостроителя(ПостроительОтчета, ТолькоПовторы = Ложь) Экспорт
	...
	Для Инд=0 По ПостроительОтчета.ИзмеренияКолонки.Количество()-1  Цикл
		...
		Для Инд2=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1  Цикл
			...
			Если ПостроительОтчета.ИзмеренияКолонки[Инд].ПутьКДанным = ПостроительОтчета.ИзмеренияСтроки[Инд2].ПутьКДанным Тогда
				...
				#Иначе
					ОбщегоНазначения.СообщитьОбОшибке("Повторяющаяся группировка " + ПостроительОтчета.ИзмеренияКолонки[Инд].Представление +"."+ Символы.ПС+
	...
	Для Инд=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1  Цикл
		...
		Иначе
			...
			#Иначе
				ОбщегоНазначения.СообщитьОбОшибке("Повторяющаяся группировка " + ПостроительОтчета.ИзмеренияСтроки[Инд].Представление +"."+ Символы.ПС+
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ РАБОТЫ С СИСТЕМОЙ ЭЛЕКТРОННОГО ДОКУМЕНТООБОРОТА (ЭДО)

// Возвращает девятизначный идентификатор клиента
//
Функция ПолучитьИдентификаторКлиента(КлиентСсылка) Экспорт
	...
	Иначе
		// Сообщаем об ошибке всеми способами :)
		ТекстСообщения = "Не найден код клиента в ЭДО для " + КлиентСсылка.Наименование;
		#Если Клиент Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Сообщить();
		#КонецЕсли
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
КонецФункции // ПолучитьИдентификаторКлиента()
