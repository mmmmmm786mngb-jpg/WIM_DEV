...
#Область ВыполнениеОтчётовИОбработокВФоне

// Добавляет информацию для глобального обработчика ожидания
//
// Параметры:
//  ОписаниеОповещения			 - ОписаниеОповещения	 - Описание обработчика получения результата выполнения
//  МаксимальныйИнтервалОпроса	 - Число				 - Желаемый интервал получения сообщений от фонового задания
//  ЭлементТабличногоДокумента	 - ПолеФормы			 - Элемент формы табличного документа, где будет отображаться состояние выполнения
//
Процедура ДобавитьОбработчикОповещения(ОписаниеОповещения, МаксимальныйИнтервалОпроса = 60, ЭлементТабличногоДокумента = Неопределено) Экспорт
	ОбработчикОжидания = НовоеОписаниеОбработчикаОжидания();
	ОбработчикОжидания.ОписаниеОповещения = ОписаниеОповещения;
	ОбработчикОжидания.ИнтервалПроверки = 1;
	ОбработчикОжидания.МаксимальныйИнтервал = МаксимальныйИнтервалОпроса;
	ПараметрыФоновогоВыполнения = ОписаниеОповещения.ДополнительныеПараметры;
	Если ЭлементТабличногоДокумента <> Неопределено
		И ЭлементТабличногоДокумента.Вид = ВидПоляФормы.ПолеТабличногоДокумента Тогда
		ОтображениеСостояния = ЭлементТабличногоДокумента.ОтображениеСостояния;
		ОтображениеСостояния.Видимость                      = Истина;
		ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
		ОтображениеСостояния.Картинка                       = БиблиотекаКартинок.ДлительнаяОперация48;
		ОтображениеСостояния.Текст                          = НСтр("ru = 'Отчет формируется...'");
		ПараметрыФоновогоВыполнения.Вставить("ОтображениеСостояния", ОтображениеСостояния);
	Иначе
		ОбщегоНазначения.СообщитьОбОшибке("Недопустимое значение параметра "+ЭлементТабличногоДокумента+"!");
	КонецЕсли;
	ОбработчикиОжидания().Добавить(ОбработчикОжидания);
	ПодключитьОбработчикОжидания("Подключаемый_ОжиданиеФоновогоВыполнения", 1, Истина);
КонецПроцедуры

Функция НовоеОписаниеОбработчикаОжидания()
	Возврат Новый Структура("ОписаниеОповещения,ИнтервалПроверки,МаксимальныйИнтервал");
КонецФункции

// Опрашивает фоновое задание и возвращает данные для обработчика оповещения об выполнении.
// При фоновом формировании отчетов меняет состояние поля табличного документа формы.
//
// Параметры:
//  ПараметрыФоновогоВыполнения	 - Структура	 - Данные о выполняемом фоновом задании
// 
// Возвращаемое значение:
//  Произвольное - Структура с результатом выполнения или информация об ошибке
//
Функция ДанныеДляОповещения(ПараметрыФоновогоВыполнения) Экспорт
	Перем ОтображениеСостояния;
	ПараметрыФоновогоВыполнения.Свойство("ОтображениеСостояния", ОтображениеСостояния);
	ОписаниеЗадания = Оболочки.СостояниеФоновогоЗадания(ПараметрыФоновогоВыполнения.ИдентификаторЗадания);
	Для Каждого Сообщение Из ОписаниеЗадания.СообщенияПользователю Цикл
		Сообщение.Сообщить();
	КонецЦикла;
	Если ОписаниеЗадания = Неопределено Или Не ЭтоАдресВременногоХранилища(ПараметрыФоновогоВыполнения.АдресХранилища) Тогда
		Возврат Неопределено;
	ИначеЕсли ОписаниеЗадания.ИнформацияОбОшибке <> Неопределено Тогда
		Если ОтображениеСостояния <> Неопределено Тогда
			...
			ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
		КонецЕсли;
		ОбщегоНазначения.СообщитьОбОшибке(ПодробноеПредставлениеОшибки(ОписаниеЗадания.ИнформацияОбОшибке));
		ПараметрыФоновогоВыполнения.АдресХранилища = "";
	ИначеЕсли Не ОписаниеЗадания.Активно Тогда
		Если ОтображениеСостояния <> Неопределено Тогда
		...
		КонецЕсли;
		РезультатИсполнения = ПолучитьИзВременногоХранилища(ПараметрыФоновогоВыполнения.АдресХранилища);
		ПараметрыФоновогоВыполнения.АдресХранилища = "";
		ПараметрыФоновогоВыполнения.ЗаданиеВыполнено = Истина;
		Возврат РезультатИсполнения;
	КонецЕсли;
	Возврат Неопределено;

КонецФункции

// Проверяет наличие активных фоновых заданий и предлагает пользователю их завершить
//
// Параметры:
//  ТекстВопроса - Строка						 - Текст предложения о завершении фонового выполнения
//  Форма		 - ФормаКлиентскогоПриложения	 - Форма (или другой адресат оповещения), для которой нужно проверить наличие активных фоновых заданий
//  Отказ		 - Булево						 - Признак наличия активных фоновых заданий и отказа пользователя их завершать
//
Процедура ПодтвердитьДействие(ТекстВопроса, Форма, Отказ) Экспорт
	Спрашивать = Истина;
	Для Каждого Обработчик Из ОбработчикиОжидания(Форма) Цикл
		ПараметрыФоновогоВыполнения = Обработчик.ОписаниеОповещения.ДополнительныеПараметры;
		Если Оболочки.ВыполняетсяФоновоеЗадание(ПараметрыФоновогоВыполнения) Тогда
			Если Спрашивать И Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Нет Тогда
				Отказ = Истина;
				Возврат;
			КонецЕсли;
			Оболочки.ОтменитьФоновоеВыполнение(ПараметрыФоновогоВыполнения);
			Спрашивать = Ложь;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
// Возвращаемое значение:
//  Массив:
//   Элементы массива - Структура (см. НовоеОписаниеОбработчикаОжидания())
//
Функция ОбработчикиОжидания(Получатель = Неопределено) Экспорт
	
	ИмяПараметра = "ФоновоеВыполнение.ОбработчикиОжидания";
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено Тогда
	...
	ИначеЕсли Получатель <> Неопределено Тогда
		Обработчики = Новый Массив;
		Для Каждого Обработчик Из ПараметрыПриложения[ИмяПараметра] Цикл
			Если Обработчик.ОписаниеОповещения.Модуль = Получатель Тогда
...
#КонецОбласти //ВыполнениеОтчётовИОбработокВФоне
