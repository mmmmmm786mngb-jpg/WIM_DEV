...
Функция ВыбратьПолучателя(Объект, МножественныйВыбор = Истина, ВозвращатьСоответствие = Истина, ЗаголовокДляОшибок = "") Экспорт
	...
	Если Не Объект.Личная = Истина Тогда
		#Если ТолстыйКлиентОбычноеПриложение Тогда
			тз = Новый ТаблицаЗначений;
			Для Каждого Ст Из Метаданные.Справочники.РассылкиОтчетов.ТабличныеЧасти.Получатели.Реквизиты Цикл
				тз.Колонки.Добавить(Ст.Имя, Ст.Тип);
			КонецЦикла;
			ПараметрыПолучателей.Вставить("Получатели", тз);	
		#Иначе
			ПараметрыПолучателей.Вставить("Получатели", Объект.Получатели.Выгрузить());	
		#КонецЕсли
	КонецЕсли;
