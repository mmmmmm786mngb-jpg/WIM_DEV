...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	ФормаОтчетаКлиентПереопределяемый.ОбработкаОповещения(ЭтаФорма, ИмяСобытия, Параметр, Источник);
КонецПроцедуры
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если Ложь Тогда
		Подключаемый_Команда(Неопределено);
	КонецЕсли;
КонецПроцедуры
...
Процедура Подключаемый_СписокСПодбором_Подбор(Команда)
	...
	ВыборТолькоГрупп = (КолонкаЗначениеЭлемент.ВыборГруппИЭлементов = ГруппыИЭлементы.Группы);
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения()
	...
	ПолеВводаВключатьКнопкуВыбора = Неопределено;
	...
	Для Каждого ПользовательскаяНастройка Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		ИначеЕсли ШаблонГенерации.Тип = "ПолеВвода" Тогда
			...
			Если ПолеВводаВключатьКнопкуВыбора = Неопределено Тогда
//				ПолеВводаВключатьКнопкуВыбора = Не ОбщегоНазначенияКлиентСервер.ЭтоПлатформа83БезРежимаСовместимости();
				ПолеВводаВключатьКнопкуВыбора = Ложь;
			КонецЕсли;
			Если ПолеВводаВключатьКнопкуВыбора Тогда
			...
			ИначеЕсли ТипЗнч(Значение) = ТипТип Тогда
				МассивТипов = Новый Массив;
	...
	Для Каждого ОписаниеРеквизита Из ДобавленныеСпискиЗначений Цикл
		...
		Если ОписаниеРеквизита.СодержитТипТип И ТипЗнч(ОписаниеРеквизита.Значение) = Тип("СписокЗначений") Тогда
			...
			Для Каждого ЭлементСпискаСКД Из ОписаниеРеквизита.Значение Цикл
				...
				Если ТипЗнч(ЭлементСписка.Значение) = ТипТип Тогда
					МассивТипов = Новый Массив;
					МассивТипов.Добавить(ЭлементСписка.Значение);
					ЭлементСписка.Значение = Новый ОписаниеТипов(МассивТипов);
				КонецЕсли;
			КонецЦикла;
			ЭтаФорма[ИмяТаблицы] = СписокЗначений;
		Иначе
			ЭтаФорма[ИмяТаблицы] = ОписаниеРеквизита.Значение;
		КонецЕсли;
		ЭтаФорма[ИмяТаблицы].ТипЗначения = ОписаниеРеквизита.ОписаниеТипов;
