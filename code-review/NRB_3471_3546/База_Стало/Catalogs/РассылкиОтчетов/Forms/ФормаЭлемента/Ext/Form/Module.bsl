...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВидРассылки.СписокВыбора.Очистить();
	Элементы.ВидРассылки.СписокВыбора.Добавить("Общая","Отчеты указанным получателям");
...
Процедура ИспользоватьЭлектроннуюПочтуПриИзменении(Элемент)
	...
	Если Объект.ИспользоватьЭлектроннуюПочту тогда
		Объект.ОтправлятьЧерезУведомленияКОтправке = Ложь;
		ВидимостьДоступностьКорректность(ЭтаФорма, "ОтправлятьЧерезУведомленияКОтправке");
	КонецЕсли;
...
Процедура ПроверитьПубликацию(Команда)
	...
	ПараметрыДоставки.Вставить("ИспользоватьУведомленияКОтправке", Ложь);
	//
...
Процедура ЗаписатьНастройкиСтрокиОтчеты(ИдентификаторСтроки)
	...
	Иначе
		...
		ТаблицаЗначений = НастройкиТекущегоОтчета.Выгрузить(); 
		СохраняемоеЗначение = ТаблицаЗначений.Скопировать(ТаблицаЗначений.НайтиСтроки(Отбор), ИменаРеквизитов);
	КонецЕсли;
...
Процедура ВидимостьДоступностьКорректность(ЭтаФорма, Изменения = "")
	...
	Если Изменения = "" ИЛИ Изменения = "ОтправлятьЧерезУведомленияКОтправке" тогда
		Если Объект.ОтправлятьЧерезУведомленияКОтправке тогда
			Элементы.ПараметрыПисьма.Доступность = Объект.ОтправлятьЧерезУведомленияКОтправке;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		ПараметрыДоставки.Вставить("УчетнаяЗапись", РассылкаОтчетов.УчетнаяЗаписьВнешнегоИсточника(Объект.ВнешнийИсточникРассылки));
...
&НаКлиенте
Процедура ОтправлятьЧерезУведомленияКОтправкеПриИзменении(Элемент)
	...
	Если Объект.ОтправлятьЧерезУведомленияКОтправке тогда
		...
		Объект.ИспользоватьЭлектроннуюПочту = Ложь;
		ВидимостьДоступностьКорректность(ЭтаФорма, "ИспользоватьЭлектроннуюПочту");
