...
// Период отбора по времени запуска фоновых (по-умолчанию час)
&НаСервере
Перем мВыводитьФоновыеЗаПоследниеСек;
...
Функция ПолучитьПолноеИмяФормы(ИмяПодчиненнойФормы) 
	Возврат Лев(ИмяФормы, Найти(ИмяФормы,".Форма.")+6) + ИмяПодчиненнойФормы; 
КонецФункции
...
Процедура ОтборПриОткрытии()
	мВыводитьФоновыеЗаПоследниеСек = 3600;
	ОтборФоновыхЗаданийВключен = Истина;
	ОтборФоновыхЗаданий = Новый ХранилищеЗначения(Новый Структура("Начало", ТекущаяДата() - мВыводитьФоновыеЗаПоследниеСек));
...
//#Область ОбработчикиПроверкиОжидания
...
//#Область УправлениеРеглЗаданиями
...
Процедура УдалитьРегламентноеЗадание()
	...
	Для Каждого Стр из ВыделенныеСтроки Цикл
		...
		РегламентноеЗадание = ПолучитьОбъектРегламентногоЗадания(Строка.Идентификатор);
...
Функция ПолучитьОбъектРегламентногоЗадания(УникальныйНомерЗадания)
	Возврат ТекущийОбъект().ПолучитьОбъектРегламентногоЗадания(УникальныйНомерЗадания);	
КонецФункции
