...
Перем ДокументыДляОтложенногоПроведения; // Таблица значений для проведения документов после загрузки данных
...
Перем ПолеТаблицаТиповСопоставленияОбъектовИнформационныхБаз;   // (кэш) ТаблицаЗначений - таблица с типами объектов,
																// которые имеют сопоставление в РС для заданного узла плана обмена
//
...
Перем СоответствиеПравилКонвертации;
...
Функция ТаблицаДанныхЗаголовкаПакета() Экспорт
	...
	Если ТипЗнч(ПолеТаблицаДанныхЗаголовкаПакета) <> Тип("ТаблицаЗначений") Тогда
		...
		Колонки.Добавить("ИмяПравила",                   одОписаниеТипа("Строка"));
...
Функция РезультатВыполненияОбменаПредупреждение()
	
	Возврат Найти(ВРег(ОбщегоНазначения.ИмяЗначенияПеречисления(РезультатВыполненияОбмена())), "ПРЕДУПРЕЖДЕНИЕ") <> 0;
...
Процедура ВыполнитьВыгрузкуДанных() Экспорт
	...
	ОбменДаннымиСобытия.ОбновитьПовторноИспользуемыеЗначенияМРОПриНеобходимости();
...
Процедура ВыполнитьЗагрузкуДанных() Экспорт
	...
	// сбрасываем кэш наличия информации сопоставления
	СброситьКэшНаличияИнформацииСопоставления();
	...
	// очищаем очередь документов к проведению
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru = 'Ошибка при загрузке данных: [ОписаниеОшибки]'");
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[УзелИнформационнойБазыНаименование]", Строка(УзелОбменаЗагрузкаДанных));
	...
	СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[РезультатВыполненияОбмена]",    РезультатВыполненияОбмена());
	СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ДействиеПриОбмене]",            Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
	СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[КоличествоОбъектовОбработано]", Формат(СчетчикЗагруженныхОбъектов(), "ЧГ=0"));
	ЗаписьЖурналаРегистрацииОбменДанными(СтрокаСообщения, УровеньЖурналаРегистрации.Информация);
...
Процедура ВыполнитьЗагрузкуДанныхВТаблицуЗначений(ТаблицыДляЗагрузки) Экспорт
	...
	Для Каждого ТипОбъектаСтрокой ИЗ ТаблицыДляЗагрузки Цикл
		
		ТаблицыДанныхСообщенияОбмена().Вставить(ТипОбъектаСтрокой, ИнициализацияТаблицыДанныхСообщенияОбмена(Тип(ТипОбъектаСтрокой)));
...
Процедура ВыполнитьАнализСообщенияОбмена() Экспорт
	...
	Попытка
		...
		// сворачиваем таблицу заголовка
		ТаблицаДанныхЗаголовкаПакета().Свернуть("ТипОбъектаСтрокой, ИмяПравила, ПоляПоиска, ПоляТаблицы, ТипИсточникаСтрокой, ТипПриемникаСтрокой, СинхронизироватьПоИдентификатору",
												"КоличествоОбъектовВИсточнике");
		
		// дополняем таблицу заголовка данными ПКО
		Для Каждого СтрокаТаблицы ИЗ ТаблицаДанныхЗаголовкаПакета() Цикл
			
			ПКО = Правила[СтрокаТаблицы.ИмяПравила];
	...
	Исключение
		СтрокаСообщения = НСтр("ru = 'Ошибка при анализе данных: [ОписаниеОшибки]'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОписаниеОшибки]", ОписаниеОшибки());
		ЗаписатьВПротоколВыполнения(СтрокаСообщения,,,,,Истина);
	КонецПопытки;
...
Функция одЗначениеЭлемента(Объект, Тип, ИскатьПоСвойству = "", ОбрезатьСтрокуСправа = Истина)
	...
	Если (Тип = ТипСтрока) ИЛИ (Тип = ТипБулево) ИЛИ (Тип = ТипЧисло) ИЛИ (Тип = ТипДата) 
		ИЛИ (Тип = ТипХранилищеЗначения) ИЛИ (Тип = ТипВидДвиженияНакопления) 
...
Функция ОткрытьФайлВыгрузки(СтрокаСообщенияОбОшибке = "")
	...
	УстановитьАтрибут(ВременныйЗаписьXML, "ВерсияФормата", "3.0");
...
Функция ЗаписатьВПротоколВыполнения(Код = "",
	...
	Если ВзвестиФлагОшибок Тогда
		...
		ПолеРезультатВыполненияОбмена = ?(РезультатВыполненияОбмена = Неопределено,
										Перечисления.РезультатыВыполненияОбмена.Ошибка,
										РезультатВыполненияОбмена);
		//
		Если РезультатВыполненияОбменаПредупреждение() Тогда
...
Процедура ЗагрузитьПравилоКонвертации(ПравилаОбмена, ЗаписьXML)
	...
	Если РежимОбмена <> "Загрузка" Тогда
		...
		СвойстваМассив = НоваяСтрока.Свойства.Скопировать(Новый Структура("ЭтоГруппа", Ложь), "Приемник").ВыгрузитьКолонку("Приемник");
		СвойстваПоискаМассив = НоваяСтрока.СвойстваПоиска.ВыгрузитьКолонку("Приемник");
		
		СвойстваПоискаМассивДополнительный = НоваяСтрока.Свойства.Скопировать(Новый Структура("ЭтоПолеПоиска", Истина), "Приемник").ВыгрузитьКолонку("Приемник");
		...
		// удаляем из массива полей поиска значение {УникальныйИдентификатор} и ЭтоГруппа
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(СвойстваПоискаМассив, "{УникальныйИдентификатор}");
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(СвойстваПоискаМассив, "ЭтоГруппа");
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип)
	...
	Попытка
		...
		Если Объект.ДополнительныеСвойства.Свойство("СоответствияОбъектовИнформационныхБаз", СоответствияОбъектовИнформационныхБаз)
			...
			РегистрыСведений.СоответствиеОбъектовИнформационныхБаз.ДобавитьЗапись(СоответствияОбъектовИнформационныхБаз);
...
Функция ПрочитатьСвойство(Тип, НеСоздаватьОбъектЕслиНеНайден = Ложь, СвойствоНеНайденоПоСсылке = Ложь)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Ссылка" Тогда
			...
			СоответствияОбъектовИнформационныхБаз = Неопределено;
			ОбъектНайден = Истина;
			Значение       = НайтиОбъектПоСсылке(Тип, , , ОбъектНайден, , НеСоздаватьОбъектЕслиНеНайден,,,,,,,СоответствияОбъектовИнформационныхБаз);
...
Функция ОпределитьПоТипуОбъектаАлгоритмПоследовательностиПолейПоиска(ТипСсылкиСтрокой)
	...
	ЗначениеСоответствия = СоответствиеАлгоритмовПоискаЭлементов.Получить(ТипСсылкиСтрокой);
	...
	Попытка
		...
		Для Каждого Элемент Из Правила Цикл
			...
			Если Элемент.Значение.Приемник = ТипСсылкиСтрокой Тогда
				...
				Если Элемент.Значение.ЕстьОбработчикПоследовательностьПолейПоиска = Истина Тогда
					...
					СоответствиеАлгоритмовПоискаЭлементов.Вставить(ТипСсылкиСтрокой, АлгоритмПоиска);
		...
		СоответствиеАлгоритмовПоискаЭлементов.Вставить(ТипСсылкиСтрокой, "");
		Возврат "";
...
Функция НайтиОбъектПоСсылке(ТипОбъекта, 
	...
	НеСоздаватьОбъектЕслиНеНайден = одАтрибут(ФайлОбмена, ТипБулево, "НеСоздаватьЕслиНеНайден");
	ПриПереносеОбъектаПоСсылкеУстанавливатьТолькоGIUD = НЕ РежимПоискаОсновногоОбъекта 
		И одАтрибут(ФайлОбмена, ТипБулево, "ПриПереносеОбъектаПоСсылкеУстанавливатьТолькоGIUD");
	...
	// 3. Поиск объекта по уникальному идентификатору ссылки (если нет поиска по полям)
	...
	ИначеЕсли ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли Тогда 
		...
		СвойствоУникальныйИдентификатор 			= Неопределено;
		СвойстваПоиска["{УникальныйИдентификатор}"] = Неопределено; 
	...
	ИначеЕсли (СвойствоУникальныйИдентификатор <> Неопределено) Тогда
		...
		НужноПродолжитьПоискЕслиЭлементПоGUIDНеНайден = ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли;
		...
		Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда 
			СвойстваОбъектаМодифицированы = Истина;    // Для записи, в т.ч. соответствия объектов ИБ
		КонецЕсли;
...
Функция ПроверитьНаличиеКоллизийДляОбъекта(Объект, ИмяТипаОбъекта, ДополнительнаяИнформация = "", ПриоритетОбъектаОбмена)
	...
	Если ЕстьКоллизия Тогда
		...
		Если ПриоритетОбъектаОбмена = Перечисления.ПриоритетыОбъектовОбмена.ПриоритетПоступившегоПриОбменеОбъектаВыше Тогда
			Комментарий = НСтр("ru = 'Возникла коллизия изменений. Объект заменен версией из файла сообщения: [Объект]'");
		Иначе
			Комментарий = НСтр("ru = 'Возникла коллизия изменений. Объект не был изменен: [Объект]'");
		КонецЕсли;
		
		Комментарий = СтрЗаменить(Комментарий, "[Объект]", Строка(Объект));
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Исключение
						...
						СтрокаСообщения = НСтр("ru = 'Ошибка при записи документа: [Объект]. Описание ошибки: [СтрокаОписанияОшибки]'");
						СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Объект]", Строка(Объект));
						СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[СтрокаОписанияОшибки]", СтрокаОписанияОшибки);
...
Процедура ВыгрузитьДанныеПоПравилу(Правило) Экспорт
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ru = 'ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: [Имя] ([Наименование])'");
		
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Имя]",          СокрЛП(Правило.Имя));
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Наименование]", СокрЛП(Правило.Наименование));
...
Процедура ВыполнитьПроверкуРеквизитовРегистрацииОбъектов(ТаблицаРеквизитовРегистрации)
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаРеквизитовРегистрации Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru = 'Тип объекта не определен: [ТипОбъектаСтрокой]'");
			
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ТипОбъектаСтрокой]", СтрокаТаблицы.ТипОбъектаСтрокой);
		...
		// проверку выполняем только для ссылочных типов: Справочник, Документ, Задача, БизнесПроцесс
		Если НЕ (ЭтоСправочник(ОбъектМД)
			 ИЛИ ЭтоДокумент(ОбъектМД)
			 ИЛИ ЭтоЗадача(ОбъектМД)
			 ИЛИ ЭтоБизнесПроцесс(ОбъектМД)) Тогда
			...
			Для Каждого Реквизит ИЗ СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если ЭтоЗадача(ОбъектМД) Тогда
					...
					Если НЕ (ОбъектМД.Реквизиты.Найти(Реквизит.Ключ) <> Неопределено
						...
						СтрокаСообщения = НСтр("ru = 'Неправильно указаны реквизиты шапки объекта ""[ОбъектМД]"". Реквизит ""[Реквизит]"" не существует'");
						
						СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМД]", Строка(ОбъектМД));
						СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Реквизит]", Реквизит.Ключ);
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
						ЗаписатьВПротоколВыполнения(СтрокаСообщения);
		...
		Иначе // реквизиты табличной части
			...
			Для Каждого Реквизит ИЗ СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если НЕ (ОбъектМД.ТабличныеЧасти[СтрокаТаблицы.ИмяТабличнойЧасти].Реквизиты.Найти(Реквизит.Ключ) <> Неопределено
					...
					СтрокаСообщения = НСтр("ru = 'Неправильно указаны реквизиты табличной части ""[ИмяТабличнойЧасти]"" объекта ""[ОбъектМД]"". Реквизит ""[Реквизит]"" не существует.'");
					
					СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяТабличнойЧасти]", СтрокаТаблицы.ИмяТабличнойЧасти);
					СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМД]",          Строка(ОбъектМД));
					СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Реквизит]",          Реквизит.Ключ);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
...
Функция ПолучитьИнформациюОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("ru = 'Правила обмена
						|Дата создания:         [ДатаВремяСоздания]
						|Конфигурация источник: [ПредставлениеКонфигурацииИсточника]
						|Конфигурация приемник: [ПредставлениеКонфигурацииПриемника]'");
	...
	СтрокаИнформации = СтрЗаменить(СтрокаИнформации, "[ДатаВремяСоздания]",                  Конвертация.ДатаВремяСоздания);
	СтрокаИнформации = СтрЗаменить(СтрокаИнформации, "[ПредставлениеКонфигурацииИсточника]", ПредставлениеКонфигурацииИсточника);
	СтрокаИнформации = СтрЗаменить(СтрокаИнформации, "[ПредставлениеКонфигурацииПриемника]", ПредставлениеКонфигурацииПриемника);
...
Процедура УдалитьОбъектПоСсылке(Ссылка, СтрокаСообщенияОбОшибке, УдалитьНепосредственно = Истина)
	...
	УдалитьОбъект(Объект, УдалитьНепосредственно);	
...
Процедура ПрочитатьУдалениеОбъекта(СтрокаСообщенияОбОшибке)
	...
	ВыполнитьЗаменуУникальногоИдентификатораПриНеобходимости(УникальныйИдентификаторСтрокой, ТипИсточникаСтрокой, ТипПриемникаСтрокой, Истина, НайденоСоответствиеОбъекта);
	...
	УдалитьОбъектПоСсылке(Ссылка, СтрокаСообщенияОбОшибке, НайденоСоответствиеОбъекта);
...
Процедура ОткрытьФайлОбмена()
	...
	Попытка
		...
		ФайлОбмена.Прочитать();
	Исключение
		СтрокаСообщенияОбОшибке = НСтр("ru = 'Ошибка при загрузке данных: [ОписаниеОшибки]'");
		СтрокаСообщенияОбОшибке = СтрЗаменить(СтрокаСообщенияОбОшибке, "[ОписаниеОшибки]", ОписаниеОшибки());
				...
				Попытка
					...
					Если Отказ = Истина Тогда
						...
						Если Не ПустаяСтрока(ПричинаОтказа) Тогда
							...
							СтрокаСообщения = НСтр("ru = 'Загрузка данных отменена по причине: [ПричинаОтказа]'");
							СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ПричинаОтказа]", ПричинаОтказа);
							ВызватьИсключение СтрокаСообщения;
						Иначе
...
Процедура ПроизвестиЧтениеДанныхДляТаблиц(ТаблицыДляЗагрузки)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Объект" Тогда
			...
			Если ТаблицыДляЗагрузки.Найти(ТипОбъектаСтрокой) <> Неопределено Тогда
				...
				Иначе // загрузка в таблицу значений
					...
					Если ПоследнийОбъектЗагрузки <> Неопределено Тогда
						...
						ТаблицаДанныхСообщенияОбмена = ТаблицыДанныхСообщенияОбмена().Получить(ТипОбъектаСтрокой);
						...
						СтрокаТаблицы[ИмяКолонкиТипСтрокой()]              = ТипОбъектаСтрокой;
			...
			ТипОбъектаСтрокойПредыдущий = ТипОбъектаСтрокой;
...
Процедура ПроизвестиЧтениеДанныхВРежимеАнализа()
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Объект" Тогда
			...
			СтрокаТаблицы.ТипОбъектаСтрокой            = одАтрибут(ФайлОбмена, ТипСтрока, "Тип");
			СтрокаТаблицы.ИмяПравила                   = одАтрибут(ФайлОбмена, ТипСтрока, "ИмяПравила");
...
Процедура ПрочитатьДанныеПоОбмену()
	...
	ИмяПланаОбмена = одАтрибут(ФайлОбмена, ТипСтрока, "ПланОбмена");
	...
	Если Не ЗначениеЗаполнено(УзелОбменаПолучатель)
		...
		СтрокаСообщения = НСтр("ru = 'Не найден узел обмена для загрузки данных. План обмена: [ИмяПланаОбмена], Код: [КодОтКого]'");
		
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]", ИмяПланаОбмена);
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[КодОтКого]", КодОтКого);
		ВызватьИсключение СтрокаСообщения;
	...
	Если УдалитьРегистрациюИзменений Тогда
		...
		НомерПринятогоСообщения = одАтрибут(ФайлОбмена, ТипЧисло, "НомерВходящегоСообщения");
	...
	Для Каждого СтрокаТаблицы ИЗ ДокументыДляОтложенногоПроведения Цикл
		...
		Попытка
			...
			Если Объект.ПроверитьЗаполнение() Тогда
				...
				НачалоЗамераПроведенияДокумента = Неопределено; ВесЗамера = 1;
				Если ТипЗнч(Объект) = Тип("ДокументОбъект.иц_СводноеПоручение") Тогда
					
					НачалоЗамераПроведенияДокумента = ОценкаПроизводительности.НачатьЗамерВремени();
					ВесЗамера = Макс(1, Объект.СоставПоручения.Количество());
				...
				Если НачалоЗамераПроведенияДокумента <> Неопределено Тогда
					
					ОценкаПроизводительности.ЗакончитьЗамерВремени("ПроведениеДокументаИц_СводноеПоручение", НачалоЗамераПроведенияДокумента, ВесЗамера);
		...
		Если Не СтрокаТаблицы.ДокументПроведенУспешно Тогда
			...
			СтрокаСообщения = НСтр("ru = 'Ошибка при отложенном проведении документа: [Объект]. Описание ошибки: [ОписаниеОшибки]'");
			
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Объект]",         Строка(Объект));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОписаниеОшибки]", ОписаниеОшибки);
			
			ЗаписьЖурналаРегистрации(КлючСообщенияЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, УзелОбменаЗагрузкаДанных.Метаданные(), Объект.Ссылка, СтрокаСообщения);
...
Процедура ЗаписатьИнформациюОбОбменеДаннымиЧерезПланыОбмена(Знач ВыгрузитьИнформациюСопоставления)
	...
	УстановитьАтрибут(Приемник, "УдалитьРегистрациюИзменений", РежимЗагрузкиДанныхВИнформационнуюБазу());
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	Если ВыгрузитьИнформациюСопоставления Тогда
		...
		Если Отказ Тогда
			...
			ЗаписатьВПротоколВыполнения(72);
		Иначе
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Для каждого СтрокаПравилаВыгрузки из ТаблицаПравилВыгрузкиИспользуемые Цикл
		...
		Пока ВыборкаИзменений.Следующий() Цикл
			...
			ИначеЕсли ВыгружаютсяКонстанты Тогда
				...
				Свойства = Менеджеры[ПравилоВыгрузкиДанных.ОбъектВыборки];
...
Функция ЭтоДокумент(ОбъектМетаданных)
	...
	Возврат ОбменДаннымиСервер.ЭтоДокумент(ОбъектМетаданных);
	
КонецФункции

Функция ЭтоСправочник(ОбъектМетаданных)
	
	Возврат ОбменДаннымиСервер.ЭтоСправочник(ОбъектМетаданных);
	
КонецФункции

Функция ЭтоБизнесПроцесс(ОбъектМетаданных)
	
	Возврат ОбменДаннымиСервер.ЭтоБизнесПроцесс(ОбъектМетаданных);
	
КонецФункции

Функция ЭтоЗадача(ОбъектМетаданных)
	
	Возврат ОбменДаннымиСервер.ЭтоЗадача(ОбъектМетаданных);
...
Процедура СброситьКэшНаличияИнформацииСопоставления()
	
	ПолеТаблицаТиповСопоставленияОбъектовИнформационныхБаз = Неопределено;
...
Процедура ИнициализацияМенеджеров()
	...
	Для каждого ОбъектМД Из Метаданные.Перечисления Цикл
		...
		ТипСсылки        = Тип(ТипСсылкиСтрокой);
		Структура = Новый Структура("Имя,ИмяТипа,ТипСсылкиСтрокой,Менеджер,ОбъектМД,ПКО,ПустаяСсылка,ВозможенПоискПоПредопределенным,СтрокаПоиска", Имя, ИмяТипа, ТипСсылкиСтрокой, Менеджер, ОбъектМД, , Перечисления[Имя].ПустаяСсылка(), Ложь, "");
		Менеджеры.Вставить(ТипСсылки, Структура);
...
Процедура ИнициализацияРеквизитовИМодульныхПеременных()
	...
	ОптимизированнаяЗаписьОбъектов = Ложь;
	...
	ДокументыДляОтложенногоПроведения.Колонки.Добавить("ДокументОбъект");
	ДокументыДляОтложенногоПроведения.Колонки.Добавить("ДокументСсылка");
	ДокументыДляОтложенногоПроведения.Колонки.Добавить("ДатаДокумента",           одОписаниеТипа("Дата"));
	ДокументыДляОтложенногоПроведения.Колонки.Добавить("ДокументПроведенУспешно", одОписаниеТипа("Булево"));
