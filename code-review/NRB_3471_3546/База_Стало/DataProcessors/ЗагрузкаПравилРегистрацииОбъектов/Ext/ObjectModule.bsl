...
Функция ПолучитьИнформациюОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("ru = 'Правила регистрации объектов
						...
						|Конфигурация:    [ПредставлениеКонфигурации]'");
	//
	СтрокаИнформации = СтрЗаменить(СтрокаИнформации, "[ДатаВремяСоздания]", Регистрация.ДатаВремяСоздания);
	СтрокаИнформации = СтрЗаменить(СтрокаИнформации, "[ИмяПланаОбмена]", Регистрация.ИмяПланаОбмена);
	СтрокаИнформации = СтрЗаменить(СтрокаИнформации, "[ПредставлениеКонфигурации]", ПолучитьПредставлениеКонфигурацииИзПравилРегистрации());
...
Функция КлючСообщенияЖурналаРегистрации()
	...
	Возврат "Обмен данными";
	
КонецФункции

////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ЗАГРУЗКИ ПРАВИЛ РЕГИСТРАЦИИ ОБЪЕКТОВ (ПРО)
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ КОМПИЛЯЦИИ ПРАВИЛ РЕГИСТРАЦИИ ОБЪЕКТОВ (ПРО)
...
Процедура ПодготовитьПравилоРегистрацииПоСвойствамПланаОбмена(ПРО)
	...
	ТекстЗапроса = ТекстВыборкиПолей + Символы.ПС 
	             ...
	             + Символы.ПС + "И НЕ ПланОбменаОсновнаяТаблица.Ссылка = &" + Регистрация.ИмяПланаОбмена + "ЭтотУзел";
	//
...
Процедура ВыполнитьПроверкуНаличияПланаОбмена()
	...
	Если Регистрация.ИмяПланаОбмена <> ИмяПланаОбменаДляЗагрузки Тогда
		...
		ОписаниеОшибки = НСтр("ru = 'В правилах регистрации указан план обмена [ИмяПланаОбмена], а загрузка выполняется для плана обмена [ИмяПланаОбменаДляЗагрузки]'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "[ИмяПланаОбмена]", Регистрация.ИмяПланаОбмена);
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "[ИмяПланаОбменаДляЗагрузки]", ИмяПланаОбменаДляЗагрузки);
