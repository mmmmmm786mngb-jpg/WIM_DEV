...
#Иначе
	...
	Данные = Неопределено;
	Прокси.ProceedAttRequest(СерверИсточник + "/" + СтрокаПараметраПолучения, Данные, "");
	Данные.Записать(ИмяВходящегоФайлаПараметр);
...
Функция СоздатьСоединениеЧерезПрокси(ОписаниеОшибкиСоединения = "")
	...
	Если ТипЗнч(HTTP) <> Тип("HTTPСоединение") Тогда
		...
		ФормаАвторизацииПрокси = ЭтотОбъект.ПолучитьФорму("Удалить_ФормаПараметрыПрокси");
...
Если ЗначениеЗаполнено(ПараметрыВнешнегоИсточника.Папка) Тогда
	Попытка
		Прокси = ВебСервисыПовтИсп.ПолучитьПроксиИнтернетСервис();
	Исключение
		ЗаписьЖурналаРегистрации("Не удалось инициировать веб-сервис", УровеньЖурналаРегистрации.Ошибка, Метаданные(), ВнешнийИсточник, КраткоеПредставлениеОшибки(ИнформацияОбОшибке())+" "+ПараметрыВнешнегоИсточника.Папка);
	КонецПопытки;
Иначе
	...
	ЗаписьЖурналаРегистрации("Не заполнены параметры внешнего источника", УровеньЖурналаРегистрации.Ошибка, Метаданные(), ВнешнийИсточник, Комментарий);
КонецЕсли;
