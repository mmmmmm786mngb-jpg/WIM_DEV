...
Функция ПериодичностьДиаграммы(ШагЧисло, КоличествоШагов) Экспорт
	...
	КоличествоШагов = 0;
	Если Шаг = "Час" Тогда
		ШагЧисло = ОбщегоНазначенияПовтИсп.СекундВЧасе();
	...
	ИначеЕсли Шаг = "День" Тогда
		ШагЧисло = ОбщегоНазначенияПовтИсп.СекундВСутках();
		КоличествоШагов = РазницаВремени / ШагЧисло;
		КоличествоШагов = Цел(КоличествоШагов) + ?(КоличествоШагов - Цел(КоличествоШагов) > 0, 1, 0);
	ИначеЕсли Шаг = "Неделя" Тогда
		ДнейВНеделе = 7;
		ШагЧисло = ОбщегоНазначенияПовтИсп.СекундВСутках(ДнейВНеделе);
		КоличествоШагов = РазницаВремени / ШагЧисло;
		КоличествоШагов = Цел(КоличествоШагов) + ?(КоличествоШагов - Цел(КоличествоШагов) > 0, 1, 0);
	ИначеЕсли Шаг = "Месяц" Тогда
		ДнейВУсловномМесяце = 30;
		ШагЧисло = ОбщегоНазначенияПовтИсп.СекундВСутках(ДнейВУсловномМесяце);
