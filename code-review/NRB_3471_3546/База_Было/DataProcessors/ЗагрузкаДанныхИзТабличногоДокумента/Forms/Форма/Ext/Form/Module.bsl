...
Процедура ОбновитьДанныеТабличногоДокумента(Знач Оповещение, БезВопросов = Ложь)
	...
	Если (Объект.РежимЗагрузки = 0 или Объект.РежимЗагрузки = 2) и Элементы.ТабДокумент.Высота > 1 И Не БезВопросов Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбновитьДанныеТабличногоДокументаЗавершение", ЭтаФорма, Новый Структура("Оповещение", Оповещение)), "Табличный документ содержит данные. Очистить?", РежимДиалогаВопрос.ДаНет);
        Возврат;
	Иначе
...
Процедура КомандаЗагрузить(Команда)
	...
	Иначе
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗагрузитьЗавершение", ЭтаФорма), "Загрузить "+КоличествоЭлементов  + СтруктураТекстВопроса.ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
