...
Процедура ЗаполнитьПараметрыОтбора()
	...
	Для Каждого ПараметрОтбора Из СписокПараметровОтбора Цикл
		...
		Если ВРег(ИмяПараметра) = ВРег("Пользователь") Тогда
			// Пользователь/User
...
Процедура ИсполнениеВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Отчет.ОтчетПоЖурналуРегистрации.Форма.РедакторСоставаСвойства", ПараметрыФормы, ЭтаФорма);
КонецПроцедуры
...
Процедура УстановитьОтборИЗакрытьФорму(Команда)
	...
	Оповестить("УстановленОтборЖурналаРегистрации",
