...
//
//
//	Порядок добавления версионирования для объекта метаданных
//	1. Указать в подписке на событие ЗаписатьВерсиюОбъекта в качестве Источника добавляемый объект метаданных
//	2. Указать в параметрах общей команды  ИсторияИзменений ссылку на объект метаданных
// 	3. Для управляемых форм элементов объекта метаданных добавить в процедуру ПриСозданииНаСервере вызов:

//	// СтандарныеПодсистемы.ВерсионированиеОбъектов
//	ВерсионированиеОбъектов.ПриСозданииНаСервере(ЭтаФорма);
//	// Конец СтандарныеПодсистемы.ВерсионированиеОбъектов

//	4. Для обычных форм элементов объекта метаданных, добавить в модуль формы процедуру:
//
//	// СтандарныеПодсистемы.ВерсионированиеОбъектов
//	Процедура ВерсионированиеОбъектовКомандаОткрытияИсторииИзмененийНажатие(Элемент)
//		ВерсионированиеОбъектовКлиент.ПоказатьИсториюИзмененийОбъекта(ЭтаФорма);
//	КонецПроцедуры
//	// Конец СтандарныеПодсистемы.ВерсионированиеОбъектов
//
//  5. Для обычных форм элементов объекта метаданных, добавить процедуру ПередОткрытием модуля формы, вызов:
//	
//	// СтандарныеПодсистемы.ВерсионированиеОбъектов
//	ВерсионированиеОбъектовКлиент.ОбычнаяФормаПередОткрытием(ЭтаФорма);
//	// Конец СтандарныеПодсистемы.ВерсионированиеОбъектов
//	6. В режиме Предприятия, установить настройки версионирования для объекта метаданных  в регистре сведений  НастройкиВерсионированияОбъектов
...
//
Процедура ПриСозданииВерсииОбъекта(ОбъектПараметр) Экспорт
	
	Перем НомерПоследнейВерсии, Комментарий;
	
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ОбъектПараметр)) Тогда
		Объект = ОбъектПараметр.ПолучитьОбъект();
	Иначе
		Объект = ОбъектПараметр;
	КонецЕсли;
...
Процедура СоздатьВерсиюОбъекта(Объект, СведенияОВерсииОбъекта, ЗаписьОбычнойВерсии = Истина)
	...
	Если СведенияОВерсииОбъекта.НомерВерсии > 1 Тогда
		ПредыдущийНабор = ЗаписьОВерсииОбъекта(Объект.Ссылка, СведенияОВерсииОбъекта.НомерВерсии - 1);
		Если ПредыдущийНабор.Количество()<>0 Тогда
			ПредыдущаяЗапись = ПредыдущийНабор[0];
			ДвоичныеДанныеПредыдущаяВерсия = ПредыдущаяЗапись.ВерсияОбъекта.Получить();
			Если ДвоичныеДанные = ДвоичныеДанныеПредыдущаяВерсия Тогда
				// Если сохраняемая версия совпадает с предыдущей версией, сохранять не будем
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Объект.ДополнительныеСвойства.Вставить("ЗаписанНомерВерсии", МенеджерЗаписиВерсииОбъектов.НомерВерсии);
...
Функция СведенияОВерсииОбъекта(знач Ссылка, знач НомерВерсии) Экспорт
	...
	Если ЕстьПравоНаЧтениеВерсийОбъектов() Тогда
		УстановитьПривилегированныйРежим(Истина);
		Выборка = Запрос.Выполнить().Выбрать();
...
Функция ВерсионированиеДоступно(Источник) Экспорт 
	
	Если Пользователи.РолиДоступны("ЧтениеВерсийОбъектов") И ПолучитьФункциональнуюОпцию("ИспользоватьВерсионированиеОбъектов") Тогда
		НомерВерсии = 0;
		Возврат ОбъектВерсионируется(Источник,НомерВерсии);
	Иначе 
		Возврат Ложь;
	КонецЕсли;
...
Процедура ВыполнитьНачальноеЗаполнениеИстории(ПараметрыВызова) Экспорт
	РегистрыСведений.ВерсииОбъектов.ВыполнитьНачальноеЗаполнениеИстории(ПараметрыВызова);
КонецПроцедуры
