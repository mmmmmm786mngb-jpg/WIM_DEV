...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	ПараметрыДоставки.Вставить("ИспользоватьУведомленияКОтправке", Рассылка.ОтправлятьЧерезУведомленияКОтправке);
	...
	Если НЕ ПараметрыДоставки.ИспользоватьПапку
		 ...
		 И НЕ ПараметрыДоставки.ИспользоватьУведомленияКОтправке
		Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту или ПараметрыДоставки.ИспользоватьУведомленияКОтправке Тогда
		ПараметрыДоставки.Вставить("УчетнаяЗапись",   УчетнаяЗаписьВнешнегоИсточника(Рассылка.ВнешнийИсточникРассылки));
...
// Процедура - Отправить отчет [табличный документ] в формате HTML как Уведомление
//
// Параметры:
//  ИсточникРассылки		 - ПланыВидовХарактеристикСсылка.ВнешниеИсточники	 - Источник рассылки отчета
//  ИмяОтчета				 - Строка											 - Название отчета
//  ТабличныйДокументОтчет	 - ТабличныйДокумент								 - Отчет [как табличный документ]
//  ДатаОтчета				 - Дата												 - Дата отчета
//
Процедура ОтправитьТабличныйДокументОтчетКакУведомлениеHTML(ИсточникРассылки, ИмяОтчета, ТабличныйДокументОтчет,
	ДатаОтчета = Неопределено) Экспорт
	
	Если ДатаОтчета = Неопределено Тогда
		ДатаФормирования = НачалоДня(ТекущаяДатаСеанса());
	Иначе
		ДатаФормирования = ДатаОтчета;
	КонецЕсли;
	
	// Сохраним файл как Excel
	ИмяФайла	= ИмяОтчета + "_" + Формат(ДатаФормирования, "ДФ=ddMMyyyy") + ".xls";
	ПутьКФайлу	= ПолучитьИмяВременногоФайла(".xls");
	ТабличныйДокументОтчет.Записать(ПутьКФайлу, ТипФайлаТабличногоДокумента.XLS97);
	
	// Прочитаем файл в тело письма как Html
	ПутьКФайлуHTML = ПолучитьИмяВременногоФайла(".HTML");
	ТабличныйДокументОтчет.Записать(ПутьКФайлуHTML, ТипФайлаТабличногоДокумента.HTML5);
	Строки = Новый Массив;
	ПрочитанныйТекст = Новый ЧтениеТекста(ПутьКФайлуHTML);
	СтрокаТекстовогоФайла = ПрочитанныйТекст.ПрочитатьСтроку();
	Строки.Добавить(СтрокаТекстовогоФайла);
	...
	ТекстПисьма = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(Строки, Символы.ПС);
	
	НастройкиОтправки = ВнешниеДанные.ПолучитьПараметрыВнешнегоИсточника(ИсточникРассылки).Настройка;
	
	Если НастройкиОтправки = Неопределено Тогда
		УровеньОшибка = УровеньЖурналаРегистрации.Предупреждение;
		ОбъектМетаданных = Метаданные.ОбщиеМодули.RuDataAPI;
		ТекстОшибки = "Не задан внешний источник [" + ИсточникРассылки + "], отправка отчета невозможна.";
		ЗаписьЖурналаРегистрации("Отправка отчетов",
			УровеньОшибка, ОбъектМетаданных, , ТекстОшибки);
	...
	ЗаписьЖурналаРегистрации("Отправка отчетов",
		УровеньЖурналаРегистрации.Примечание, ОбъектМетаданных, , ИмяОтчета + " - отправлен");
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	Если Рассылка.Личная тогда
		ТаблицаПолучателей.Колонки.Найти("Получатель").ТипЗначения.Типы().Добавить( ТипЗнч(Рассылка.Автор));	
	КонецЕсли;
	...
	Иначе
		...
		ТипПолучателей = Рассылка.Ссылка.ТипПолучателейРассылки.КлючОбъектаМетаданных.Получить();
		//#Если ТолстыйКлиентОбычноеПриложение Тогда			
			Для каждого СтрокаТЧ Из Рассылка.Ссылка.Получатели Цикл
				НовСтрока = ТаблицаПолучателей.Добавить();
	...
	Если ТипПолучателей = Тип("СправочникСсылка.Пользователи") Тогда
		...
		|	Неопределено КАК НастройкаУведомления
		|ИЗ
	...
	Иначе
		...
		ОтправлятьЧерезУведомленияКОтправке = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Рассылка.Ссылка, "ОтправлятьЧерезУведомленияКОтправке");
			...
			|	ТаблицаПолучателей.Получатель,
			|	ТаблицаПолучателей.Исключен,
			|	ТаблицаПолучателей.НастройкаУведомления
			|ПОМЕСТИТЬ втТаблицаПолучателей
			|ИЗ
			|	&ТаблицаПолучателей КАК ТаблицаПолучателей
			|;
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	КонтактнаяИнформацияСрезПоследних.Объект КАК Получатель,
			|	ЕСТЬNULL(НастройкиОбменаДанными.АдресПолучателя, КонтактнаяИнформацияСрезПоследних.Представление) КАК EMail,
			|	втТаблицаПолучателей.НастройкаУведомления КАК НастройкаУведомления
			|ИЗ
			|	втТаблицаПолучателей КАК втТаблицаПолучателей
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НастройкиОбменаДанными КАК НастройкиОбменаДанными
			|		ПО втТаблицаПолучателей.НастройкаУведомления = НастройкиОбменаДанными.Ссылка
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация.СрезПоследних(, Вид = &ВидКИ) КАК КонтактнаяИнформацияСрезПоследних
			|			ПО Контрагенты.ЮридическиеФизическиеЛица = КонтактнаяИнформацияСрезПоследних.Объект
			|		ПО втТаблицаПолучателей.Получатель = Контрагенты.Ссылка";
	...
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ВидКИ", "&ТипКИ");
	...
	Для Каждого СтрИтог Из ТЗИтог Цикл
		Если НЕ ЗначениеЗаполнено(СтрИтог.EMail) и Не ЗначениеЗаполнено(СтрИтог.НастройкаУведомления)  Тогда
			Продолжить;
		КонецЕсли;
		ТекущийАдрес = СписокПолучателей.Получить(СтрИтог.Получатель);
		Если ТекущийАдрес = Неопределено Тогда
			Если ОтправлятьЧерезУведомленияКОтправке тогда
				НастройкаТранспорта = СтрИтог.НастройкаУведомления;
			Иначе 
				НастройкаТранспорта = СтрИтог.EMail;
			КонецЕсли;
			СписокПолучателей.Вставить(СтрИтог.Получатель, НастройкаТранспорта);
		Иначе
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Иначе // ПараметрыОтчета.СКД 
		...
		Если ЗначениеЗаполнено(ПараметрыОтчета.Настройки) Тогда
			...
			НастройкаТаблица =( ТипЗнч(ПараметрыОтчета.Настройки) = Тип("ТаблицаЗначений"));
			Для Каждого КлючИЗначение Из ПараметрыОтчета.Настройки Цикл
				Если НастройкаТаблица тогда
					Настройка = КлючИЗначение.Реквизит;
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьУведомленияКОтправке Тогда
			...
			Попытка
				ОтправитьОтчетыПолучателю(ВложенияПисьма, ПараметрыДоставки,,Истина);
...
// Возвращает учетную запись электронной почты внешнего источника
//
// Параметры:
...
//
Функция УчетнаяЗаписьВнешнегоИсточника(ВнешнийИсточник) Экспорт
	НастройкиВнешнегоИсточника = ВнешниеДанные.ПолучитьПараметрыВнешнегоИсточника(ВнешнийИсточник).Настройка;
	Если ЗначениеЗаполнено(НастройкиВнешнегоИсточника) Тогда
		УчетнаяЗапись = Справочники.НастройкиОбменаДанными.АктуальнаяУчетнаяЗаписьПриемаОтправкиСообщений(НастройкиВнешнегоИсточника);
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого ФорматСохранения Из ПараметрыОтчета.Форматы Цикл
		
		ПараметрыФормата = ПараметрыДоставки.ПараметрыФорматов.Получить(ФорматСохранения);
		...
		СтрокаФайл.Настройки.Формат = ФорматСохранения;
...
Функция ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Для Каждого ФорматМД Из Метаданные.Перечисления.ФорматыСохраненияОтчетов.ЗначенияПеречисления Цикл
		ФорматСохранения = Перечисления.ФорматыСохраненияОтчетов[ФорматМД.Имя];
		ПараметрыФормата = ПараметрыЗаписиТабличногоДокументаВФормат(ФорматСохранения);
		ПараметрыФормата.Вставить("Имя", ФорматМД.Имя);
		ПараметрыДоставки.ПараметрыФорматов.Вставить(ФорматСохранения, ПараметрыФормата);
	КонецЦикла;
...
//
Процедура ОтправитьОтчетыПолучателю(Вложения, ПараметрыДоставки, СтрокаПолучатель = Неопределено, ЧерезУведомления = Ложь)
	...
	Если ПараметрыПисьма.Свойство("ПредустановленныеВложения") Тогда
		Для Каждого КлючИЗначение Из ПараметрыПисьма.ПредустановленныеВложения Цикл
			...
			УправлениеЭлектроннойПочтой.ПривестиВложениеДляВставкиВПисьмо(Вложение);
	...
	Если Получатель = Неопределено Тогда
		...
		Если ПараметрыДоставки.ЗаполнитьПолучателяВШаблонеТемы ИЛИ ПараметрыДоставки.ЗаполнитьПолучателяВШаблонеСообщения Тогда
			...
			Для Каждого КлючИЗначение Из ПараметрыДоставки.Получатели Цикл
				...
				// Отправляем письмо
				Если ЧерезУведомления тогда
					ЗарегистрироватьСообщениеКУведомлению(ПараметрыПисьма);
				иначе
		...
		Иначе
			...
			Если ЧерезУведомления тогда
				Кому = Новый Массив;
				Для Каждого КлючИЗначение Из ПараметрыДоставки.Получатели Цикл
					Кому.Добавить(КлючИЗначение.Значение);
				КонецЦикла;
...
Функция ЗарегистрироватьСообщениеКУведомлению(знач ПараметрыПисьма) Экспорт
	...
	Если ПараметрыПисьма.Свойство("Вложения") тогда
		Вложения = Новый Соответствие;
		Для каждого КиЗ Из ПараметрыПисьма.Вложения Цикл
			ИмяВложения = КиЗ.Ключ;
			Вложения.Вставить(ИмяВложения, Новый ДвоичныеДанные(КиЗ.Значение));
		КонецЦикла; 
	КонецЕсли; 
		
	НастройкиОбмена = Неопределено;
	Если ПараметрыПисьма.Свойство("Кому") тогда
		НастройкиОбмена = ПараметрыПисьма.Кому;
	ИначеЕсли ПараметрыПисьма.Свойство("СлепыеКопии") тогда
		НастройкиОбмена = ПараметрыПисьма.СлепыеКопии;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(НастройкиОбмена) Тогда
		ОписаниеОшибки = "Не заполнены параметры внешнего источника для '"++"'";
		ЗаписьЖурналаРегистрации("Онлайн загрузка сделок. Рассылка логов загрузки",
	...
	Для каждого Настройка Из НастройкиОбмена Цикл
		...
		ПараметрыУведомления = Новый Соответствие;
		ПараметрыУведомления.Вставить("НастройкиОбмена"		, Настройка);
...
Функция ОтправитьПочтовоеСообщение(знач УчетнаяЗапись,
	...
	Настройка = Новый Структура("УчетнаяЗаписьПриемаОтправкиСообщений,АдресПолучателя", УчетнаяЗапись, ПараметрыПисьма.Кому);
	УправлениеЭлектроннойПочтой.ОтправитьСообщениеПолучателям(Настройка, ПараметрыПисьма.Тема, ПараметрыПисьма.Тело, ПараметрыПисьма.Вложения);
...
Процедура АрхивацияВложений(Вложения, ПараметрыДоставки, КаталогВременныхФайлов)
	...
	Если ПараметрыДоставки.Свойство("ЗаполнитьСформированныеОтчетыВШаблонеСообщения") И ПараметрыДоставки.ЗаполнитьСформированныеОтчетыВШаблонеСообщения Тогда
		ПараметрыДоставки.ПредставлениеОтчетовПолучателя = 
			ПараметрыДоставки.ПредставлениеОтчетовПолучателя 
			+ Символы.ПС 
			+ Символы.ПС
	...
	Если ПараметрыДоставки.ДобавлятьСсылки = "КАрхиву" Тогда
		...
		КонецЕсли;
	ИначеЕсли ПараметрыДоставки.Свойство("ЗаполнитьСформированныеОтчетыВШаблонеСообщения") И ПараметрыДоставки.ЗаполнитьСформированныеОтчетыВШаблонеСообщения Тогда
