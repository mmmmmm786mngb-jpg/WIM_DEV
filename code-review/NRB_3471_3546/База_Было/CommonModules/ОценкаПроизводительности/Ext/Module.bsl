...
Процедура УстановитьЦелевоеВремя (КлючевыеОперации) Экспорт
	...
	Попытка
		...
		Если Метаданные.РежимУправленияБлокировкойДанных 
			= Метаданные.СвойстваОбъектов.РежимУправленияБлокировкойДанныхПоУмолчанию.Управляемый Тогда
...
Процедура ОчиститьРегистрыЗамерыВремени() Экспорт
	...
	ЧислоСутокХранения = Константы.ПериодХраненияЗамеров.Получить();
	ГраницаУдаления = НачалоДня(ТекущаяУниверсальнаяДата() - ОбщегоНазначенияПовтИсп.СекундВСутках(ЧислоСутокХранения));
