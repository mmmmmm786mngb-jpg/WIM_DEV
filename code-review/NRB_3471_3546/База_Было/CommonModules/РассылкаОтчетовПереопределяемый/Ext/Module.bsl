...
Процедура ПереопределитьТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы) Экспорт
	...
	ТипПользователь = ТаблицаТипов.Найти(тип("СправочникСсылка.Пользователи"), "");
	Если ТипПользователь <> Неопределено тогда
		ТаблицаТипов.Удалить(ТипПользователь);
	КонецЕсли;
	
	СтрокиУдалить = Новый Массив;
	Для каждого СтрокатТЧ Из ТаблицаТипов Цикл
	
		Если  СтрокатТЧ.ТипПолучателей.СодержитТип(Тип("СправочникСсылка.Пользователи")) тогда
			СтрокиУдалить.Добавить(СтрокатТЧ);
		КонецЕсли;
