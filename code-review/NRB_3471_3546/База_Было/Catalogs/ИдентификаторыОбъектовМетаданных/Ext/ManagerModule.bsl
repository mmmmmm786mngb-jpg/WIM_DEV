...
Процедура ВыполнитьОбновлениеДанных(ЕстьИзменения = Ложь, ЕстьУдаленные = Ложь, ТолькоПроверка = Ложь, ЕстьКритичныеИзменения = Ложь,
			ТребуетсяОбновитьКэш = Ложь, СписокКритичныхИзменений = "") Экспорт
	
	ПроверкаИспользования();
	
	УстановитьПривилегированныйРежим(Истина);
	
	ЗаменитьДублиПодчиненногоУзлаНайденныеПриЗагрузке(ТолькоПроверка, ЕстьИзменения);
	...
	// При отключении режима совместимости отключена блокировка 
	// внутри управляемой транзакции, т.к. с версии 8.3.3 установка блокировки в конфигурациях с 
	// автоматическим режимом приводит к выбросу исключения
