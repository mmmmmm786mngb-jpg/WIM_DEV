...
Процедура ПользователиИнфБазыПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ПользователиИнфБазы.ТекущиеДанные.КодПроблемы = 0 Тогда
		...
		Иначе
			...
			НСтр("ru = 'Записать пользователя информационной базы в справочник?'"), РежимДиалогаВопрос.ДаНет);	
			
			Если ОтветДополнительный = КодВозвратаДиалога.Да Тогда
				
				СтруктураНовогоПользователя = Новый Структура("Имя, ПолноеИмя, ИдентификаторПользователяИБ");
				ЗаполнитьЗначенияСвойств(СтруктураНовогоПользователя, Элементы.ПользователиИнфБазы.ТекущиеДанные);
				
				СоздатьПользователяПоДаннымИнфБазы(СтруктураНовогоПользователя, Отказ);
...
&НаСервере
Процедура СоздатьПользователяПоДаннымИнфБазы(СтруктураНовогоПользователя, Отказ)
	
	ОбъектПользователь = Справочники.Пользователи.СоздатьЭлемент();
	ОбъектПользователь.Код          				= СтруктураНовогоПользователя.Имя;
	ОбъектПользователь.Наименование 				= СтруктураНовогоПользователя.ПолноеИмя;
	ОбъектПользователь.ИдентификаторПользователяИБ 	= СтруктураНовогоПользователя.ИдентификаторПользователяИБ;
	...
	Если ТекущиеДанные.КодПроблемы = 0 Тогда
		Предупреждение(
			НСтр("ru = 'Данному пользователю информационной базы не сопоставлен элемент справочника.
			           ...
			           |связанного с пользователем применяйте флажок 
